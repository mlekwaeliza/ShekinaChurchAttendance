// The query layer: one method per data operation, extracted from the former
// monolithic database.js so schema/boot code and data access can be reviewed
...
// createQueries returns the exact object shape that database.js used to export
// as `queries`. Dependencies (run/get/all/usePostgres) are injected so this
// module stays free of connection and schema concerns.

function createQueries({ run, get, all, usePostgres }) {
  return {
    // Auth queries
    findUserByUsername: (username) =>
      get(
        `
    SELECT u.*, 
      l.is_head, l.id AS leader_id, s.id AS section_id, s.name AS section_name,
      cl.id AS children_leader_id, cl.is_head AS children_is_head
    FROM users u 
    LEFT JOIN leaders l ON u.id = l.user_id 
    LEFT JOIN sections s ON l.section_id = s.id 
    LEFT JOIN children_leaders cl ON u.id = cl.user_id 
    WHERE u.username = ?
  `,
        [username]
      ),
    ...
    lockUser: (userId, until) =>
      run('UPDATE users SET locked_until = ?, lockout_count = lockout_count + 1 WHERE id = ?', [
        until,
        userId
      ]),
    isUserLocked: (userId) =>
      get('SELECT locked_until, lockout_count FROM users WHERE id = ?', [userId]),
  ...
  `
    ...
    validatePasswordResetToken: (tokenHash) =>
      get(
        `
  ...
  `,
    ...
    createUser: (username, password_hash, role, full_name, profile_picture = null) =>
      run(
        ...
        [username, password_hash, role, full_name, profile_picture]
      ),
    ...
    updateUserFullName: (full_name, userId, member_id = null) =>
      member_id
        ? run(
            ...
            [full_name, member_id, userId]
          )
        : run('UPDATE users SET full_name = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [
            full_name,
            userId
          ]),
  ...
  `,
    ...
    updateUserUsername: (userId, username) =>
      run('UPDATE users SET username = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [
        username,
        userId
      ]),
  ...
  `,
    ...
    createLeader: (userId, sectionId, phone, email, is_head = 0) =>
      run(
        ...
        [userId, sectionId, phone, email, is_head]
      ),
    updateLeaderInfo: (leaderId, sectionId, phone, email, is_head = 0) =>
      run('UPDATE leaders SET section_id = ?, phone = ?, email = ?, is_head = ? WHERE id = ?', [
        sectionId,
        phone,
        email,
        is_head,
    ...
    deleteUserAndCascade: (userId) => run('DELETE FROM users WHERE id = ?', [userId]),

    // Children's Ministry Leaders queries
    getChildrenLeaderByUserId: (userId) =>
      get(
        `
    SELECT cl.*, u.username, u.full_name, u.role
  ...
  `,
    ...
    getChildrenLeaderById: (leaderId) =>
      get(
        `
    SELECT cl.*, u.username, u.full_name, u.role
    FROM children_leaders cl
  ...
  `),
    createChildrenLeader: (userId, phone, email, is_head = 0) =>
      run('INSERT INTO children_leaders (user_id, phone, email, is_head) VALUES (?, ?, ?, ?)', [
        userId,
        phone,
        email,
        is_head
      ]),
    updateChildrenLeader: (leaderId, phone, email, is_head, is_active) =>
      run(
        'UPDATE children_leaders SET phone = ?, email = ?, is_head = ?, is_active = ? WHERE id = ?',
        [phone, email, is_head, is_active, leaderId]
      ),
    deleteChildrenLeader: (leaderId) =>
      run('DELETE FROM children_leaders WHERE id = ?', [leaderId]),
    getChildrenByLeader: (leaderId) =>
      all(
        `
  ...
  `,
    ...
    getChildrenSubmissionLog: (leaderId, startDate, endDate) =>
      all(
        `
    SELECT csl.*, cl.name as class_name
    FROM children_submission_log csl
    LEFT JOIN children_classes cl ON csl.class_id = cl.id
    WHERE csl.children_leader_id = ? AND csl.date BETWEEN ? AND ?
  ...
  `,
    ...
    getDashboardComparisons: (monthStart = formatLocalDate(addMonths(new Date(), -1))) =>
      all(
        `
    SELECT 
  ...
  `,
    ...
    `,
      ...
      const eligibleServices = services.filter(
        (s) => (datesByService.get(Number(s.id)) || []).length >= 3
      );
      ...
      // member.
      const cutoff = eligibleServices.map((s) => datesByService.get(Number(s.id))[2]).sort()[0];
      ...
      if (visitorKeys.length > 0) {
        const visitorMemberIds = Array.from(
          new Set(visitorKeys.map((k) => Number(k.split('_')[0])))
        );
        const visitorServiceIds = Array.from(
          new Set(visitorKeys.map((k) => Number(k.split('_')[1])))
        );
        ...
        for (const r of visitorRows) {
          visitorPresentCounts.set(
            `${Number(r.member_id)}_${Number(r.service_type_id)}`,
            Number(r.c)
          );
        }
      }
      ...
      for (const a of absentMembersByService.values()) {
        ...
        const allThree =
          knownDates.length === 3 && knownDates.every((d) => a._absentDates.includes(d));
        if (!allThree) continue;

        // Opt-out check.
        let optOuts = [];
        try {
          optOuts = JSON.parse(a.opt_out_services || '[]');
        } catch (_) {
          /* noop */
        }
    ...
    getAttendanceSparkline: (
      serviceId = 1,
      startDate = formatLocalDate(addDays(new Date(), -28))
    ) => {
      const isAll = String(serviceId) === 'all';
      const query = `
  ...
  `),
    ...
    updateServiceType: (id, name, day, time, rules, points) =>
      run(
        `UPDATE service_types SET name = ?, default_day = ?, default_time = ?, eligibility_rules = ?, points_config = ? WHERE id = ?`,
        [name, day, time, rules, points, id]
      ),

    // Service Instances / Assignments
    getServiceInstance: (serviceId, date) =>
      get('SELECT * FROM service_instances WHERE service_id = ? AND date = ?', [serviceId, date]),
    ...
    getAssignedInstancesByLeader: (leaderId) =>
      all(
        `
  ...
  `),
    ...
    createMember: (
      ...
      run(
        `
      INSERT INTO members (membership_id, full_name, section_id, leader_id, phone, email, gender, marital_status, occupation, age_group, date_of_birth, show_age_to_leaders, hide_from_birthday_list, opt_out_services, address, profile_picture, education_level, secondary_phone)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `,
        [
          membershipId,
    ...
    updateMember: (
      ...
      run(
        `
      UPDATE members
      SET full_name = ?, phone = ?, email = ?, gender = ?, marital_status = ?, occupation = ?, age_group = ?, date_of_birth = ?, show_age_to_leaders = ?, hide_from_birthday_list = ?, opt_out_services = ?, address = ?, section_id = ?, leader_id = ?, profile_picture = COALESCE(?, profile_picture), education_level = ?, secondary_phone = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `,
        [
          fullName,
    ...
    updateMemberPhoto: (memberId, photoUrl) =>
      run('UPDATE members SET profile_picture = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [
        photoUrl,
        memberId
      ]),
    deleteMember: (id) => run('DELETE FROM members WHERE id = ?', [id]),
    getMemberByMembershipId: (membershipId) =>
      get('SELECT * FROM members WHERE membership_id = ?', [membershipId]),
    findActiveMemberByName: (fullName, excludeMemberId = null) =>
      get(
        `
  ...
  `,
    ...
    // Attendance queries
    getAttendanceByLeaderAndDate: (leaderId, date) =>
      all(
        `
  ...
  `,
    ...
    checkSubmissionExists: (leaderId, date, serviceId) =>
      get('SELECT * FROM submission_log WHERE leader_id = ? AND date = ? AND service_id = ?', [
        leaderId,
        date,
        serviceId
      ]),
    logSubmission: (leaderId, sectionId, date, serviceId = 1) =>
      run(
        ...
        [leaderId, sectionId, date, serviceId]
      ),
    ...
    LIMIT 30
  `),
    getLeaderSectionAttendanceStats: (leaderId, startDate, endDate) =>
      all(
        `
    SELECT
      a.date,
  ...
  `,
    ...
    getSectionAttendanceStats: (startDate, endDate) =>
      all(
        `
    SELECT
      s.name as section_name,
  ...
  `,
    ...
    getLeaderMetrics: (startDate, endDate) =>
      all(
        `
    SELECT
      u.full_name as leader_name,
  ...
  `,
    ...
    getAtRiskMembersByRange: (
      startDate = formatLocalDate(addDays(new Date(), -30)),
  ...
  `,
    ...
    getSubmissionCompletion: (startDate, endDate) =>
      all(
        `
    SELECT
      s.name as section_name,
  ...
  `,
    ...
    updateUserProfilePicture: (profilePicturePath, userId) =>
      run('UPDATE users SET profile_picture = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [
        profilePicturePath,
        userId
      ]),
    ...
    updateUser2FA: (userId, totpSecret, totpEnabled, backupCodes) =>
      run(
        'UPDATE users SET totp_secret = ?, totp_enabled = ?, backup_codes = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
        [totpSecret, totpEnabled ? 1 : 0, backupCodes ? JSON.stringify(backupCodes) : null, userId]
      ),
    getUser2FA: (userId) =>
      get('SELECT id, totp_secret, totp_enabled, backup_codes FROM users WHERE id = ?', [userId]),
    disableUser2FA: (userId) =>
      run(
        'UPDATE users SET totp_secret = NULL, totp_enabled = 0, backup_codes = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
        [userId]
      ),
    ...
    FROM weekly_totals
  `),
    getSectionAnomalies: (threshold = 20, startDate = formatLocalDate(addDays(new Date(), -90))) =>
      all(
        `
  ...
  `,
    ...
    getLeaderPerformanceTrends: (startDate, endDate) =>
      all(
        `
  ...
  `,
    ...
    getDemographicBreakdown: (startDate = formatLocalDate(addDays(new Date(), -90))) =>
      all(
        `
    SELECT
  ...
  `,
    ...
    getYearOverYear: (currentYear = new Date().getFullYear()) =>
      all(
        `
  ...
  `,
    ...
    getServiceTypeBreakdown: (days = 90) =>
      all(
        `
    SELECT
      st.id,
  ...
  `,
    ...
    getAttendanceDayPatterns: (days = 180) =>
      all(
        `
    SELECT
  ...
  `,
    ...
    getMonthlyAttendanceContribTrends: (months = 12) =>
      all(
        `
    SELECT
  ...
  `,
    ...
    getMonthlyContributionTrends: (months = 12) =>
      all(
        `
    SELECT
  ...
  `,
    ...
    getMonthlySectionTrends: (months = 6) =>
      all(
        `
    SELECT
      ${yearMonth('a.date')} as month,
  ...
  `),
    ...
    getComparisonAnalytics: (period1Start, period1End, period2Start, period2End) =>
      get(
        `
    SELECT
  ...
  `,
    ...
    // ── Historical Stats ──────────────────────────────────────────────────
    getHistoricalStats: (startDate, endDate) =>
      get(
        `
    SELECT
  ...
  `,
    ...
    getHistoricalDaily: (startDate, endDate) =>
      all(
        `
    SELECT
      date,
  ...
  `,
    ...
    getLeaderRankings: (days = 90, startDate, endDate, prevStartDate, prevEndDate) => {
      ...
      SELECT
        ...
        SUM(CASE WHEN a.status = 'absent' THEN 1 ELSE 0 END) as total_absent,
        SUM(CASE WHEN a.status = 'excused' THEN 1 ELSE 0 END) as total_excused,
        COALESCE(ROUND(CAST(AVG(CASE WHEN a.status = 'present' THEN 1.0 ELSE 0.0 END) * 100 AS NUMERIC), 1), 0) as attendance_rate,
        (
          SELECT ROUND(CAST(AVG(CASE WHEN ap.status = 'present' THEN 1.0 ELSE 0.0 END) * 100 AS NUMERIC), 1)
          FROM attendance ap
          JOIN members mp ON (mp.leader_id = l.id OR (COALESCE(l.is_head, 0) = 1 AND mp.section_id = l.section_id))
          WHERE mp.is_active = 1 AND ap.date >= ? AND ap.date <= ?
        ) as prev_rate,
        (
          SELECT COUNT(DISTINCT a2.member_id)
          FROM attendance a2
          JOIN members m2 ON (m2.leader_id = l.id OR (COALESCE(l.is_head, 0) = 1 AND m2.section_id = l.section_id))
          WHERE m2.is_active = 1 AND a2.status = 'present' AND a2.date >= ? AND a2.date <= ?
        ) as unique_attendees,
        (
          ...
          WHERE (mr.leader_id = l.id OR (COALESCE(l.is_head, 0) = 1 AND mr.section_id = l.section_id)) AND mr.is_active = 1
        ) as retention_rate,
        (
          SELECT COUNT(DISTINCT sub.date)
          FROM submission_log sub
          WHERE (sub.leader_id = l.id OR (COALESCE(l.is_head, 0) = 1 AND sub.section_id = l.section_id)) AND sub.date >= ? AND sub.date <= ?
        ) as submissions_count,
        (
          SELECT ROUND(
            CAST((SELECT COUNT(DISTINCT sub.date) FROM submission_log sub WHERE (sub.leader_id = l.id OR (COALESCE(l.is_head, 0) = 1 AND sub.section_id = l.section_id)) AND sub.date >= ? AND sub.date <= ?) AS NUMERIC) /
        ...
        (
          ...
          WHERE (af.leader_id = l.id OR (COALESCE(l.is_head, 0) = 1 AND af.member_id IN (SELECT id FROM members WHERE section_id = l.section_id))) AND af.absence_date >= ? AND af.absence_date <= ?
        ) as follow_up_required,
        (
          SELECT COUNT(*)
          FROM absent_followups af
          WHERE (af.leader_id = l.id OR (COALESCE(l.is_head, 0) = 1 AND af.member_id IN (SELECT id FROM members WHERE section_id = l.section_id))) AND af.contacted = 1 AND af.absence_date >= ? AND af.absence_date <= ?
      ...
      JOIN users u ON l.user_id = u.id
      JOIN sections s ON l.section_id = s.id
      LEFT JOIN members m ON (m.leader_id = l.id OR (COALESCE(l.is_head, 0) = 1 AND m.section_id = l.section_id))
      LEFT JOIN attendance a ON a.member_id = m.id AND a.date >= ? AND a.date <= ?
      WHERE l.is_active = 1
    ...
    getMemberIntelligence: (days = 90, startDate, endDate, serviceId = 'all') => {
      ...
      const rangeDays = Math.max(
        1,
        Math.floor((new Date(`${end}T12:00:00`) - new Date(`${start}T12:00:00`)) / 86400000) + 1
      );
  ...
  `,
    ...
    getMemberStreakDetails: (memberId) =>
      get(
        `
    SELECT
      (SELECT COUNT(*) FROM (
  ...
  `,
    ...
    getAttendanceTrendsWithMA: (weeks = 26) =>
      all(
        `
    SELECT
      date,
  ...
  `,
    ...
    getCorrelationAnalytics: (months = 6) =>
      all(
        `
    SELECT
      ${yearMonth('a.date')} as month,
  ...
  `),
    ...
    createNotification: (userId, type, title, message, entityType = null, entityId = null) =>
      run(
        ...
        try {
          ...
          /* bus not loaded yet (tests) */
        }
        return res;
      }),
    getUnreadCount: (userId) =>
      get('SELECT COUNT(*) as count FROM notifications WHERE user_id = ? AND is_read = 0', [
        userId
      ]),
    ...
    getNotifications: (userId, limit = 50) =>
      all('SELECT * FROM notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT ?', [
        userId,
        limit
      ]),
    getAllNotifications: (userId) =>
      all('SELECT * FROM notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT 500', [
        userId
      ]),
    ...
    getConsecutiveAbsentMembers: (leaderId, minStreak = 2) =>
      all(
        `
  ...
  `,
    ...
    getFollowUpByMemberAndDate: (memberId, absenceDate) =>
      get('SELECT * FROM absent_followups WHERE member_id = ? AND absence_date = ?', [
        memberId,
        absenceDate
      ]),
    ...
    getMemberAuditHistory: (memberId) =>
      all(
  ...
  `),
    ...
    createOutreachLog: (leaderId, memberId, contactMethod, message, weekStart) =>
      run(
        'INSERT INTO outreach_logs (leader_id, member_id, contact_method, message, week_start) VALUES (?, ?, ?, ?, ?)',
        [leaderId, memberId, contactMethod, message, weekStart]
      ),
    getOutreachByLeaderAndWeek: (leaderId, weekStart) =>
      all(
    ...
    getOutreachByLeader: (leaderId, limit = 50) =>
      all(
    ...
    getAllOutreachForWeek: (weekStart) =>
      all(
  ...
  `,
    ...
    getLeaderOutreachMembersNotContacted: (leaderId, weekStart) =>
      all(
        `
  ...
  `,
    ...
    getMembersNeedingFollowUp: (startDate = formatLocalDate(addDays(new Date(), -14))) =>
      all(
        `
    SELECT m.id, m.full_name, m.membership_id, m.phone,
           s.name as section_name,
  ...
  `,
    ...
    getTodayBirthdays: (todayMonthDay = formatMonthDay()) =>
      all(
        `
     SELECT m.id, m.full_name, m.membership_id, m.phone, m.date_of_birth,
            s.name as section_name,
   ...
   `,
    ...
    createScheduledReminder: (type, entityType, entityId, scheduledFor, payload) =>
      run(
        'INSERT INTO scheduled_reminders (type, entity_type, entity_id, scheduled_for, payload) VALUES (?, ?, ?, ?, ?)',
    ...
    markReminderSent: (reminderId) =>
      run('UPDATE scheduled_reminders SET sent = 1, sent_at = CURRENT_TIMESTAMP WHERE id = ?', [
        reminderId
      ]),
    getUpcomingReminders: (type, hours = 24) =>
      all(
    ...
    getActiveTitles: () =>
      all(
        'SELECT * FROM congregation_titles WHERE is_active = 1 ORDER BY sort_order ASC, name ASC'
      ),
    getTitleById: (id) => get('SELECT * FROM congregation_titles WHERE id = ?', [id]),
    createTitle: (name, description = null, sort_order = 0) =>
      run('INSERT INTO congregation_titles (name, description, sort_order) VALUES (?, ?, ?)', [
        name,
        description,
        sort_order
      ]),
  ...
  `,
    ...
    assignMemberTitle: (memberId, titleId, assignedBy, appointmentDate = null, notes = null) =>
      run(
        'INSERT INTO member_titles (member_id, title_id, assigned_by, appointment_date, notes) VALUES (?, ?, ?, ?, ?) ON CONFLICT(member_id, title_id) DO NOTHING',
        [memberId, titleId, assignedBy, appointmentDate, notes]
      ),
    updateMemberTitle: (memberId, titleId, status, notes) =>
      run('UPDATE member_titles SET status = ?, notes = ? WHERE member_id = ? AND title_id = ?', [
        status,
        notes,
        memberId,
        titleId
  ...
  `,
    ...
    addMemberTitleHistory: (
      ...
      run(
        'INSERT INTO member_title_history (member_id, title_id, action, changed_by, old_status, new_status, old_notes, new_notes, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
        [memberId, titleId, action, changedBy, oldStatus, newStatus, oldNotes, newNotes, notes]
      ),
  ...
  `),
    createTitleFull: (name, description, category, sort_order, reports_to_title_id, is_active) =>
      run(
        'INSERT INTO congregation_titles (name, description, category, sort_order, reports_to_title_id, is_active) VALUES (?, ?, ?, ?, ?, ?)',
        [
          name,
          description || null,
          category || 'General',
    ...
    updateTitleFull: (
      ...
      run(
        'UPDATE congregation_titles SET name = ?, description = ?, category = ?, sort_order = ?, reports_to_title_id = ?, is_active = ? WHERE id = ?',
        [
          name,
          description || null,
          category || 'General',
  ...
  `,
    ...
    createDepartment: (
      ...
      run(
        'INSERT INTO departments (name, description, reports_to_title_id, leader_id, assistant_leader_id, secretary_id) VALUES (?, ?, ?, ?, ?, ?)',
        [
          name,
          description || null,
          reports_to_title_id || null,
    ...
    updateDepartment: (
      ...
      run(
        'UPDATE departments SET name = ?, description = ?, reports_to_title_id = ?, leader_id = ?, assistant_leader_id = ?, secretary_id = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
        [
          name,
          description || null,
          reports_to_title_id || null,
  ...
  `,
    ...
    addDepartmentMember: (departmentId, memberId) =>
      run(
        'INSERT INTO department_members (department_id, member_id) VALUES (?, ?) ON CONFLICT(department_id, member_id) DO NOTHING',
        [departmentId, memberId]
      ),
    removeDepartmentMember: (departmentId, memberId) =>
      run('DELETE FROM department_members WHERE department_id = ? AND member_id = ?', [
        departmentId,
        memberId
      ]),
    removeDepartmentMemberById: (id) => run('DELETE FROM department_members WHERE id = ?', [id]),

    // Department history
    addDepartmentHistory: (departmentId, memberId, role, action, notes, changedBy) =>
      run(
        'INSERT INTO department_history (department_id, member_id, role, action, notes, changed_by) VALUES (?, ?, ?, ?, ?, ?)',
  ...
  `,
    ...
    // ── New Member Leader ──────────────────────────────────────────────────
    getNewMembers: (status = 'probation') =>
      all(
        `
  ...
  `,
        [
          ...
          data.address || null,
          data.date_joined,
          data.decision_type || null,
          data.marital_status || null,
          data.date_of_birth || null,
          data.occupation || null,
          data.invitation_source || null,
          data.added_by || null,
          data.mentor_id || null,
          data.notes || null
        ]
      ),
    updateNewMember: (id, data) =>
      run(
        `
  ...
  `,
    ...
    graduateNewMember: (id, sectionId, graduatedBy) =>
      run(
        `
  ...
  `,
    ...
    getNewMemberAttendance: (newMemberId) =>
      all(
        `
  ...
  `,
    ...
    getNewMembersAttendanceByWeek: (weekStart) =>
      all(
        `
  ...
  `,
    ...
    upsertNewMemberAttendance: (newMemberId, weekStart, attended, notes, recordedBy) =>
      run(
        usePostgres
          ? `
  ...
  `,
    ...
    // Reports
    getNewMembersReport: (startDate, endDate) =>
      all(
        usePostgres
          ? `
  ...
  `),
    ...
    getPipelineMembers: (stage = null) => {
      const sql = stage
        ? `SELECT nm.*, u.full_name as added_by_name, m.full_name as mentor_name,
                s.name as section_name, hc.name as home_cell_name,
                d.name as ministry_department_name
         FROM new_members nm
         LEFT JOIN users u ON nm.added_by = u.id
         LEFT JOIN members m ON nm.mentor_id = m.id
         LEFT JOIN sections s ON nm.graduated_to_section_id = s.id
         LEFT JOIN home_cells hc ON nm.home_cell_id = hc.id
         LEFT JOIN departments d ON nm.ministry_department_id = d.id
         WHERE nm.pipeline_stage = ?
         ORDER BY nm.date_joined DESC`
        : `SELECT nm.*, u.full_name as added_by_name, m.full_name as mentor_name,
                s.name as section_name, hc.name as home_cell_name,
                d.name as ministry_department_name
    ...
    ${stage === 'permanent' ? ", status = 'permanent', is_active = 0, graduation_date = CURRENT_DATE" : ''}
    ${stage === 'graduation_review' ? ", status = 'graduated'" : ''}
    WHERE id = ?
  `,
        [stage, id]
      ),
    recordStageTransition: (newMemberId, stage, notes, userId) =>
      run(
        usePostgres
          ? `
    INSERT INTO new_member_stages (new_member_id, stage, notes, recorded_by)
    VALUES ($1, $2, $3, $4)
  `
          : `
    INSERT INTO new_member_stages (new_member_id, stage, notes, recorded_by)
    VALUES (?, ?, ?, ?)
  `,
        [newMemberId, stage, notes || null, userId || null]
      ),
    getMemberJourney: (newMemberId) =>
      all(
        `
    SELECT nms.*, u.full_name as recorded_by_name
    FROM new_member_stages nms
    LEFT JOIN users u ON nms.recorded_by = u.id
    WHERE nms.new_member_id = ?
    ORDER BY nms.stage_date ASC
  `,
        [newMemberId]
      ),
    addFollowup: (newMemberId, type, notes, nextDate, conductedBy) =>
      run(
        usePostgres
          ? `
    INSERT INTO new_member_followups (new_member_id, followup_type, notes, next_followup_date, conducted_by)
    VALUES ($1, $2, $3, $4, $5)
  `
          : `
    INSERT INTO new_member_followups (new_member_id, followup_type, notes, next_followup_date, conducted_by)
    VALUES (?, ?, ?, ?, ?)
  `,
        [newMemberId, type, notes || null, nextDate || null, conductedBy || null]
      ),
    getMemberFollowups: (newMemberId) =>
      all(
        `
    SELECT nmf.*, u.full_name as conducted_by_name
    FROM new_member_followups nmf
    LEFT JOIN users u ON nmf.conducted_by = u.id
    WHERE nmf.new_member_id = ?
    ORDER BY nmf.followup_date DESC
  `,
        [newMemberId]
      ),
  ...
  `),
    getPipelineTasks: () => {
      ...
      return all(
        `
      SELECT 'missed_orientation' as task_type, nm.id, nm.full_name, nm.pipeline_stage,
             'Missed Orientation' as title,
             'Member has not attended orientation yet' as description,
             'high' as priority
      FROM new_members nm
      WHERE nm.pipeline_stage = 'received'
        AND nm.date_joined <= date('now', '-7 days')
      UNION ALL
      SELECT 'no_home_cell' as task_type, nm.id, nm.full_name, nm.pipeline_stage,
             'No Home Cell Assigned' as title,
             'Orientation completed but no home cell assigned' as description,
             'medium' as priority
      FROM new_members nm
      WHERE nm.pipeline_stage = 'orientation_completed'
      UNION ALL
      SELECT 'no_section' as task_type, nm.id, nm.full_name, nm.pipeline_stage,
             'No Section Assigned' as title,
             'Home cell assigned but no section yet' as description,
             'medium' as priority
      FROM new_members nm
      WHERE nm.pipeline_stage = 'home_cell_assigned'
      UNION ALL
      SELECT 'no_mentor' as task_type, nm.id, nm.full_name, nm.pipeline_stage,
             'No Mentor Assigned' as title,
             'Section assigned but no mentor yet' as description,
             'medium' as priority
      FROM new_members nm
      WHERE nm.pipeline_stage = 'section_assigned'
      UNION ALL
      SELECT 'inactive' as task_type, nm.id, nm.full_name, nm.pipeline_stage,
             'Inactive 2+ Weeks' as title,
             'No attendance recorded in 2 weeks' as description,
             'high' as priority
      FROM new_members nm
      WHERE nm.pipeline_stage NOT IN ('permanent')
        AND nm.id NOT IN (SELECT new_member_id FROM new_member_attendance WHERE week_start >= ?)
      UNION ALL
      SELECT 'ready_graduation' as task_type, nm.id, nm.full_name, nm.pipeline_stage,
             'Ready for Graduation' as title,
             'All criteria met for permanent membership' as description,
             'high' as priority
      FROM new_members nm
      WHERE nm.pipeline_stage = 'ministry_placement'
      ORDER BY priority DESC
    `,
        [twoWeeksAgo]
      );
    },
    transferBaptizedToNewMembers: (soulWonId, userId) => {
      return (async () => {
        const soul = await get('SELECT * FROM souls_won WHERE id = ?', [soulWonId]);
        if (!soul) throw new Error('Soul won not found');
        const baptism = await get(
          "SELECT * FROM baptism_tracking WHERE soul_won_id = ? AND status = 'completed'",
          [soulWonId]
        );
        const baptismDate = baptism?.baptism_date || null;
        const existing = await get('SELECT id FROM new_members WHERE soul_won_id = ?', [soulWonId]);
        if (existing) return existing;
        const result = await run(
          `
        INSERT INTO new_members (full_name, phone, date_joined, decision_type, added_by, soul_won_id, baptism_date, pipeline_stage, status)
        VALUES (?, ?, CURRENT_DATE, 'Salvation', ?, ?, 'received', 'probation')
      `,
          [soul.full_name, soul.phone || null, userId || null, soulWonId, baptismDate]
        );
        const newMemberId = result.lastID || result.id;
        await run(
          usePostgres
            ? `
        INSERT INTO new_member_stages (new_member_id, stage, notes, recorded_by)
        VALUES ($1, 'received', 'Auto-transferred from Evangelism (baptism completed)', $2)
      `
            : `
        INSERT INTO new_member_stages (new_member_id, stage, notes, recorded_by)
        VALUES (?, 'received', 'Auto-transferred from Evangelism (baptism completed)', ?)
      `,
          [newMemberId, userId || null]
        );
        await run("UPDATE souls_won SET follow_up_status = 'active_member' WHERE id = ?", [
          soulWonId
        ]);
        return { id: newMemberId, full_name: soul.full_name };
      })();
    },
    getBaptizedAwaitingTransfer: () =>
      all(`
    SELECT sw.id, sw.full_name, sw.phone, sw.date_saved, bt.baptism_date
    FROM souls_won sw
    JOIN baptism_tracking bt ON bt.soul_won_id = sw.id AND bt.status = 'completed'
    WHERE sw.follow_up_status = 'baptized'
      AND sw.id NOT IN (SELECT soul_won_id FROM new_members WHERE soul_won_id IS NOT NULL)
    ORDER BY bt.baptism_date DESC
  `),
    getAssimilationFunnel: () =>
      all(
        usePostgres
          ? `
    SELECT
      (SELECT COUNT(*) FROM outreach_events) as outreach_events,
      (SELECT COUNT(*) FROM souls_won) as souls_won,
      (SELECT COUNT(*) FROM souls_won WHERE follow_up_status = 'baptized' OR follow_up_status = 'active_member') as baptized,
      (SELECT COUNT(*) FROM new_members WHERE pipeline_stage = 'received' OR pipeline_stage IS NULL) as received,
      (SELECT COUNT(*) FROM new_members WHERE pipeline_stage IN ('orientation_scheduled','orientation_in_progress','orientation_completed')) as orientation_started,
      (SELECT COUNT(*) FROM new_members WHERE pipeline_stage = 'graduation_review') as graduation_review,
      (SELECT COUNT(*) FROM new_members WHERE pipeline_stage = 'permanent') as permanent
  `
          : `
    SELECT
      (SELECT COUNT(*) FROM outreach_events) as outreach_events,
  ...
  `,
    ...
    `;
      const params = [];
      if (filters.status) {
        sql += ' AND sw.follow_up_status = ?';
        params.push(filters.status);
      }
      if (filters.start_date) {
        sql += ' AND sw.date_saved >= ?';
  ...
  `,
        [
          ...
          data.location,
          data.date_saved,
          data.outreach_event_id,
          data.soul_winner,
          data.follow_up_status || 'new_convert',
          data.assigned_leader_id,
          data.created_by
        ]
      ),
    updateSoulWon: (id, data) =>
      run(
        `
  ...
  `,
        [
          ...
          data.follow_up_officer,
          data.home_visit_status,
    ...
    updateEvangelismFollowUp: (id, data) =>
      run(
        `
  ...
  `,
    ...
    getEvangelismTeam: () =>
      all('SELECT * FROM evangelism_team WHERE is_active = 1 ORDER BY souls_won DESC'),
    createEvangelismTeamMember: (data) =>
      run(
        `
  ...
  `,
    ...
    updateEvangelismTeamMember: (id, data) =>
      run(
        `
  ...
  `,
    ...
    getEvangelismStats: async () => {
      ...
      const soulsMonth = await get(
        'SELECT COUNT(*) as count FROM souls_won WHERE date_saved >= ?',
        [monthStartStr]
      );
      const soulsYear = await get('SELECT COUNT(*) as count FROM souls_won WHERE date_saved >= ?', [
        yearStart
      ]);
      const weekStart = new Date();
      weekStart.setDate(weekStart.getDate() - weekStart.getDay() + 1);
      const weekStartStr = weekStart.toISOString().split('T')[0];
      const newConvertsWeek = await get(
        'SELECT COUNT(*) as count FROM souls_won WHERE date_saved >= ? AND follow_up_status = ?',
        [weekStartStr, 'new_convert']
      );
      const newConvertsMonth = await get(
        'SELECT COUNT(*) as count FROM souls_won WHERE date_saved >= ? AND follow_up_status = ?',
        [monthStartStr, 'new_convert']
      );
      const baptisms = await get(
        'SELECT COUNT(*) as count FROM baptism_tracking WHERE status = ?',
        ['completed']
      );
      const activeOutreach = await get(
        'SELECT COUNT(*) as count FROM outreach_events WHERE date >= ?',
        [monthStartStr]
      );
      const pendingFollowUps = await get(
        'SELECT COUNT(*) as count FROM souls_won WHERE follow_up_status IN (?,?)',
        ['new_convert', 'under_follow_up']
      );
      const totalVisitors = await get('SELECT COUNT(*) as count FROM souls_won');
      const totalSaved = await get(
        "SELECT COUNT(*) as count FROM souls_won WHERE follow_up_status != 'new_convert'"
      );
      const conversionRate =
        totalVisitors.count > 0 ? ((totalSaved.count / totalVisitors.count) * 100).toFixed(1) : 0;
  ...
  `
    ...
    getConversionFunnel: async () => {
      ...
      const followedUp = await get(
        "SELECT COUNT(*) as count FROM souls_won WHERE follow_up_status IN ('under_follow_up','joined_cell','joined_church','baptized','active_member')"
      );
      const baptized = await get(
        'SELECT COUNT(DISTINCT soul_won_id) as count FROM baptism_tracking WHERE status = ?',
        ['completed']
      );
      const members = await get(
        "SELECT COUNT(*) as count FROM souls_won WHERE follow_up_status IN ('active_member','joined_church')"
      );
      return {
        visitors: visitors.count,
        saved: saved.count,
        followed_up: followedUp.count,
        baptized: baptized.count,
        members: members.count
      };
    },
    getEvangelismMonthlyReport: (year) =>
      all(
        usePostgres
          ? `
  ...
  `,
    ...
    createContributionType: ({ name, description, sort_order }) =>
      run('INSERT INTO contribution_types (name, description, sort_order) VALUES (?, ?, ?)', [
        name,
        description,
        sort_order
      ]),
    ...
    getContributions: (filters = {}) => {
      ...
      if (filters.payment_method) {
        WHERE.push('c.payment_method=?');
        params.push(filters.payment_method);
      }
      if (filters.date_from) {
        WHERE.push('c.payment_date>=?');
  ...
  `,
    ...
    updateContribution: (id, data) => {
      ...
      if (data.contribution_type_id !== undefined) {
        SET.push('contribution_type_id=?');
        params.push(data.contribution_type_id);
      }
    ...
    getContributionsByDateRange: (from, to) =>
      all(
        `
  ...
  `,
        [
          data.record_date,
          data.morning_offering,
          data.afternoon_offering,
          data.total_tithes,
          data.total_income,
          data.mission_fund,
          data.remaining_after_mission,
          data.bishop_fund,
          data.usable_church_funds,
          data.evangelism_offering || 0,
          data.notes,
          data.created_by
        ]
      ),
    ...
    updateFinanceRecord: (id, data) => {
      ...
      if (data.record_date !== undefined) {
        SET.push('record_date=?');
        params.push(data.record_date);
      }
      if (data.morning_offering !== undefined) {
        SET.push('morning_offering=?');
        params.push(data.morning_offering);
      }
      if (data.afternoon_offering !== undefined) {
        SET.push('afternoon_offering=?');
        params.push(data.afternoon_offering);
      }
      if (data.total_tithes !== undefined) {
        SET.push('total_tithes=?');
        params.push(data.total_tithes);
      }
      if (data.total_income !== undefined) {
        SET.push('total_income=?');
        params.push(data.total_income);
      }
      if (data.mission_fund !== undefined) {
        SET.push('mission_fund=?');
        params.push(data.mission_fund);
      }
      if (data.remaining_after_mission !== undefined) {
        SET.push('remaining_after_mission=?');
        params.push(data.remaining_after_mission);
      }
      if (data.bishop_fund !== undefined) {
        SET.push('bishop_fund=?');
        params.push(data.bishop_fund);
      }
      if (data.usable_church_funds !== undefined) {
        SET.push('usable_church_funds=?');
        params.push(data.usable_church_funds);
      }
      ...
      if (data.rejection_reason !== undefined) {
        SET.push('rejection_reason=?');
        params.push(data.rejection_reason);
      }
      if (data.evangelism_offering !== undefined) {
        SET.push('evangelism_offering=?');
        params.push(data.evangelism_offering);
      }
      if (data.notes !== undefined) {
        SET.push('notes=?');
        params.push(data.notes);
      ...
      if (data.bishop_receipt !== undefined) {
        SET.push('bishop_receipt=?');
        params.push(data.bishop_receipt);
      }
      if (data.evangelism_receipt !== undefined) {
        SET.push('evangelism_receipt=?');
        params.push(data.evangelism_receipt);
      }
      if (data.remaining_receipt !== undefined) {
        SET.push('remaining_receipt=?');
        params.push(data.remaining_receipt);
      }
      if (SET.length === 0) return Promise.resolve({ changes: 0 });
      return run(
        `UPDATE finance_daily_records SET ${SET.join(', ')}, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
        [...params, id]
      );
    },
    getEvangelismMonthlyReport: (year) =>
  ...
  `,
    ...
    approveFinanceRecord: (id, userId) =>
      run(
        `
  ...
  `,
    ...
    rejectFinanceRecord: (id, userId, reason) =>
      run(
        `
  ...
  `,
    ...
    updateFinanceExpenseReceipt: (id, receiptPath) =>
      run(
        `
