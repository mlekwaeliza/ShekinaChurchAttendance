...
async function ensureHomeCellSchema() {
  ...
  if (usePostgres) {
    ...
    try {
      ...
      `);
      await run('CREATE INDEX IF NOT EXISTS idx_nm_stages_member ON new_member_stages(new_member_id)');
      ...
      `);
      await run('CREATE INDEX IF NOT EXISTS idx_nm_followups_member ON new_member_followups(new_member_id)');
    } catch (e) {
...
async function migrateUsersRoleConstraint() {
  try {
    if (usePostgres) {
      ...
      await run(`ALTER TABLE users ADD CONSTRAINT users_role_check CHECK (role IN ('admin', 'leader', 'pastor', 'evangelist', 'accountant', 'children_leader'))`);
...
async function createChildrensMinistryTables() {
  try {
    ...
    const childrenCols = await all(usePostgres
...
const queries = {
  ...
  getChildrenLeaderByUserId: (userId) => get(`
    SELECT cl.*, u.username, u.full_name, u.role
    FROM children_leaders cl
    JOIN users u ON cl.user_id = u.id
    WHERE cl.user_id = ?
  `, [userId]),
  getChildrenLeaderById: (leaderId) => get(`
    SELECT cl.*, u.username, u.full_name, u.role
    FROM children_leaders cl
    JOIN users u ON cl.user_id = u.id
    WHERE cl.id = ?
  `, [leaderId]),
  getAllChildrenLeaders: () => all(`
    SELECT cl.*, u.username, u.full_name, u.role
    FROM children_leaders cl
    JOIN users u ON cl.user_id = u.id
  ...
  createChildrenLeader: (userId, phone, email, is_head = 0) =>
    run('INSERT INTO children_leaders (user_id, phone, email, is_head) VALUES (?, ?, ?, ?)', [userId, phone, email, is_head]),
  updateChildrenLeader: (leaderId, phone, email, is_head, is_active) =>
    run('UPDATE children_leaders SET phone = ?, email = ?, is_head = ?, is_active = ? WHERE id = ?', [phone, email, is_head, is_active, leaderId]),
  deleteChildrenLeader: (leaderId) =>
    run('DELETE FROM children_leaders WHERE id = ?', [leaderId]),
  getChildrenByLeader: (leaderId) => all(`
  ...
  createMember: (membershipId, fullName, sectionId, leaderId, phone, email, gender, maritalStatus, occupation, ageGroup, dob = null, showAge = 0, hideBday = 0, optOuts = '[]', address = null, profilePicture = null, educationLevel = null, secondaryPhone = null) =>
    run(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [membershipId, fullName, sectionId, leaderId, phone, email, gender, maritalStatus, occupation, ageGroup, dob, showAge, hideBday, optOuts, address, profilePicture, educationLevel, secondaryPhone]),
  updateMember: (fullName, phone, email, gender, maritalStatus, occupation, ageGroup, dob, showAge, hideBday, optOuts, address, sectionId, leaderId, memberId, profilePicture = null, educationLevel = null, secondaryPhone = null) =>
    run(`
      UPDATE members
      ...
      WHERE id = ?
    `, [fullName, phone, email, gender, maritalStatus, occupation, ageGroup, dob, showAge, hideBday, optOuts, address, sectionId, leaderId, profilePicture, educationLevel, secondaryPhone, memberId]),
  ...
  PIPELINE_STAGES: [
    'received', 'orientation_scheduled', 'orientation_in_progress', 'orientation_completed',
    'home_cell_assigned', 'section_assigned', 'mentor_assigned', 'ministry_placement',
    'graduation_review', 'permanent'
  ],
  ...
  updatePipelineStage: (id, stage, userId) => run(`
    ...
    WHERE id = ?
  `, [stage, id]),
  recordStageTransition: (newMemberId, stage, notes, userId) => run(usePostgres ? `
  ...
  getMemberJourney: (newMemberId) => all(`
  ...
  `, [newMemberId]),
  addFollowup: (newMemberId, type, notes, nextDate, conductedBy) => run(usePostgres ? `
  ...
  getMemberFollowups: (newMemberId) => all(`
  ...
  transferBaptizedToNewMembers: (soulWonId, userId) => {
    return (async () => {
      ...
      const baptism = await get('SELECT * FROM baptism_tracking WHERE soul_won_id = ? AND status = \'completed\'', [soulWonId]);
      ...
      await run('UPDATE souls_won SET follow_up_status = \'active_member\' WHERE id = ?', [soulWonId]);
      return { id: newMemberId, full_name: soul.full_name };
    })();
  },
  getBaptizedAwaitingTransfer: () => all(`
  ...
  `),
  getAssimilationFunnel: () => all(usePostgres ? `
    SELECT
