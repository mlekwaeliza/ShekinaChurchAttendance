  ...
  if (usePostgres) {
    ...
    await run(`
      CREATE TABLE IF NOT EXISTS new_members (
        ...
        status TEXT DEFAULT 'probation' CHECK(status IN ('probation', 'graduated', 'permanent')),
        graduation_date DATE,
        graduated_to_section_id INTEGER REFERENCES sections(id) ON DELETE SET NULL,
        graduated_by INTEGER REFERENCES users(id) ON DELETE SET NULL,
        notes TEXT,
        is_active INTEGER DEFAULT 1,
        created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
    ...
    await run(`
      CREATE TABLE IF NOT EXISTS new_member_attendance (
        id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
        new_member_id INTEGER NOT NULL REFERENCES new_members(id) ON DELETE CASCADE,
        week_start DATE NOT NULL,
        attended INTEGER DEFAULT 0,
        notes TEXT,
