    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-amber-600 via-orange-600 to-rose-500 p-8 text-white shadow-xl shadow-amber-500/20">
        ...
        <div className="relative z-10">
          ...
          <h2 className="text-2xl font-bold tracking-tight mt-1">Evangelism Department Report</h2>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        >
          <UserPlus className="w-4 h-4" /> Register New Convert
        </button>
        <button
          onClick={() => setShowEventModal(true)}
          className="btn-primary flex items-center gap-2"
        >
          <Calendar className="w-4 h-4" /> Create Outreach Event
        </button>
      </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Saved:{' '}
                      {soul.date_saved ? new Date(soul.date_saved).toLocaleDateString() : 'N/A'}
                    </p>
                  </div>
                </div>
                <span
                  className={`text-xs font-medium px-2.5 py-1 rounded-full ${
                    soul.follow_up_status === 'new_convert'
...
const OutreachEvents = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await evangelismAPI.deleteOutreachEvent(id);
      load();
    } catch (err) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="input w-full"
                placeholder="e.g. Crusade, Door-to-Door"
              />
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Volunteers</label>
              <input
                type="number"
            ...
            <div className="sm:col-span-2">
              ...
              <input
                ...
                className="input w-full"
                placeholder="e.g. 15 souls won"
              />
            </div>
          </div>
          <div className="flex gap-3">
            <button type="submit" className="btn-primary">
              {editing ? 'Update' : 'Save'} Event
            </button>
            <button
              type="button"
              onClick={() => {
          ...
          events.map((ev) => (
            <div key={ev.id} className={STAT_STYLE}>
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="h-9 w-9 rounded-lg inline-flex items-center justify-center hover:bg-slate-100 text-slate-400 hover:text-primary-600"
                  >
                    <Edit3 className="w-4 h-4" />
                  </button>
                  <button
                    onClick={() => handleDelete(ev.id)}
                    className="h-9 w-9 rounded-lg inline-flex items-center justify-center hover:bg-rose-50 text-slate-400 hover:text-rose-600"
                  >
              ...
              <div className="mt-3 flex flex-wrap gap-3 text-xs text-slate-500">
                {ev.event_type && (
                  <span className="px-2 py-1 rounded-full bg-slate-100">{ev.event_type}</span>
                )}
                {ev.organizer && (
                  <span>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-slate-600">
                      {s.date_saved ? new Date(s.date_saved).toLocaleDateString() : '-'}
                    </td>
                    ...
                    <td className="py-3 px-4">
                      ...
                      >
                        {status?.label || s.follow_up_status}
                      </span>
                    </td>
                      ...
                      <button
                        onClick={() => {
                          ...
                          setForm({
                            ...
                            location: s.location || '',
                            date_saved: s.date_saved?.split('T')[0] || '',
                            outreach_event_id: s.outreach_event_id || '',
                            soul_winner: s.soul_winner || '',
                            follow_up_status: s.follow_up_status || 'new_convert',
                            assigned_leader_id: s.assigned_leader_id || ''
                          });
                          setShowForm(true);
                        }}
                        className="h-9 w-9 rounded-lg inline-flex items-center justify-center hover:bg-slate-100 text-slate-400 hover:text-primary-600"
                      ...
                      <button
                        onClick={async () => {
                          if (window.confirm('Delete this record?')) {
                            try {
                              await evangelismAPI.deleteSoulWon(s.id);
                              load();
                            } catch (err) {
                              alert('Failed to delete');
                            }
...
const FollowUpManagement = () => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setSouls(
        res.data.filter((s) => ['new_convert', 'under_follow_up'].includes(s.follow_up_status))
      );
    } catch (err) {
      console.error(err);
    } finally {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {showForm && (
        <form onSubmit={handleSave} className={STAT_STYLE + ' space-y-4'}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Soul Won *</label>
              <select
                value={form.soul_won_id}
                onChange={(e) => setForm({ ...form, soul_won_id: e.target.value })}
                className="input w-full"
                required
              >
                <option value="">Select...</option>
            ...
            <div>
              ...
              <input
                type="text"
                value={form.follow_up_officer}
                onChange={(e) => setForm({ ...form, follow_up_officer: e.target.value })}
                className="input w-full"
              />
            </div>
            <div>
              ...
              <input
                type="text"
                value={form.home_visit_status}
                onChange={(e) => setForm({ ...form, home_visit_status: e.target.value })}
                className="input w-full"
            ...
            <div>
              ...
              <input
                type="text"
                value={form.counseling_status}
                onChange={(e) => setForm({ ...form, counseling_status: e.target.value })}
                className="input w-full"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Prayer Needs</label>
              <input
                type="text"
                value={form.prayer_needs}
                onChange={(e) => setForm({ ...form, prayer_needs: e.target.value })}
                className="input w-full"
              />
            </div>
            <div className="sm:col-span-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Saved:{' '}
                      {soul.date_saved ? new Date(soul.date_saved).toLocaleDateString() : 'N/A'}{' '}
                      {soul.soul_winner ? `\u00B7 ${soul.soul_winner}` : ''}
                    </p>
                  </div>
                </div>
                {expanded === soul.id ? (
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700 space-y-3">
                  {(followUps[soul.id] || []).length === 0 ? (
                    ...
                    followUps[soul.id].map((fu) => (
                      <div
                        key={fu.id}
                        className="p-3 rounded-xl bg-slate-50 dark:bg-slate-700/50 text-sm"
                      >
                        <div className="flex justify-between text-slate-500 mb-1">
                          <span>
                            <UserCheck className="w-3 h-3 inline mr-1" />
                            {fu.follow_up_officer || 'Unassigned'}
                          </span>
                          <span>
                            <Clock className="w-3 h-3 inline mr-1" />
                            {fu.last_contact_date
                              ? new Date(fu.last_contact_date).toLocaleDateString()
                              : 'No contact'}
                          </span>
                        </div>
                        {fu.home_visit_status && (
                          <span className="inline-block mr-2 text-xs px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">
                            {fu.home_visit_status}
                          </span>
                        )}
                        {fu.counseling_status && (
                          <span className="inline-block text-xs px-2 py-0.5 rounded-full bg-cyan-100 text-cyan-700">
                            {fu.counseling_status}
                          </span>
                        )}
                        {fu.prayer_needs && (
...
const EvangelismTeam = () => {
  ...
  const load = useCallback(async () => {
    try {
      const res = await evangelismAPI.getTeam();
      setTeam(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editing) {
        await evangelismAPI.updateTeamMember(editing.id, form);
      } else {
        await evangelismAPI.createTeamMember(form);
      }
      setShowForm(false);
      setEditing(null);
      setForm({
      ...
      {showForm && (
        <form onSubmit={handleSave} className={STAT_STYLE + ' space-y-4'}>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Souls Won</label>
              <input
                type="number"
                value={form.souls_won}
                onChange={(e) => setForm({ ...form, souls_won: parseInt(e.target.value) || 0 })}
                className="input w-full"
              />
            </div>
          </div>
            ...
            {[...team]
              ...
              .map((member, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <span
                      className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${i === 0 ? 'bg-amber-100 text-amber-600' : i === 1 ? 'bg-slate-200 text-slate-600' : i === 2 ? 'bg-orange-100 text-orange-600' : 'bg-slate-100 text-slate-500'}`}
                    >
                      {i + 1}
                    </span>
                    <div>
              ...
              >
                ...
                <td className="py-3 px-4">
                  <span className="text-xs px-2 py-1 rounded-full bg-primary-100 text-primary-700">
                    {roleOptions.find((o) => o.value === m.role)?.label || m.role}
                  </span>
                </td>
                  ...
                  <button
                    onClick={async () => {
                      if (window.confirm('Remove this member?')) {
                        try {
                          await evangelismAPI.deleteTeamMember(m.id);
                          load();
                        } catch (err) {
                          alert('Failed to delete');
                        }
...
const BaptismTracking = () => {
  ...
  const load = useCallback(async () => {
    try {
      const res = await evangelismAPI.getBaptismRecords();
      setRecords(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editing) {
        await evangelismAPI.updateBaptismRecord(editing.id, form);
      } else {
        await evangelismAPI.createBaptismRecord(form);
      }
      setShowForm(false);
      setEditing(null);
      setForm({
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, salvation_date: e.target.value })}
                className="input w-full"
              />
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Baptized By</label>
              <input
                type="text"
                value={form.baptized_by}
                onChange={(e) => setForm({ ...form, baptized_by: e.target.value })}
                className="input w-full"
              />
            </div>
            <div>
              ...
              records.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-slate-600">
                      {r.salvation_date ? new Date(r.salvation_date).toLocaleDateString() : '-'}
                    </td>
                    <td className="py-3 px-4 text-slate-600">
                      {r.baptism_date ? new Date(r.baptism_date).toLocaleDateString() : '-'}
                    </td>
                    ...
                    <td className="py-3 px-4 text-right">
                      <button
                        onClick={() => {
                          setEditing(r);
                          setForm({
                            soul_won_id: r.soul_won_id || '',
                            candidate_name: r.candidate_name,
                            salvation_date: r.salvation_date?.split('T')[0] || '',
                            baptism_date: r.baptism_date?.split('T')[0] || '',
                            baptized_by: r.baptized_by || '',
                            status: r.status
                          });
                          setShowForm(true);
                        }}
                      ...
                      <button
                        onClick={async () => {
                          if (window.confirm('Delete this record?')) {
                            try {
                              await evangelismAPI.deleteBaptismRecord(r.id);
                              load();
                            } catch {
                              alert('Failed to delete');
                            }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <select
          ...
          className="input w-32 self-start"
        >
              ...
              {monthly.length === 0 ? (
                ...
                monthly.map((row, i) => (
                  <tr key={i} className="border-b border-slate-100 dark:border-slate-700">
                    ...
                    <td className="py-3 px-4 text-right text-primary-600 font-bold">
                      {row.members_added || 0}
                    </td>
                  </tr>
                ))
