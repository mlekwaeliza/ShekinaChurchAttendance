...
const useAdminData = () => {
  ...
  const [filterValue, setFilterValue] = useState(() =>
    getSearchValue('period', getWeekString(new Date()))
  );
  ...
  const handleMemberSectionFilter = useCallback(
    (sectionName) => {
  ...
  const handleMemberLeaderFilter = useCallback(
    (leaderName) => {
  ...
  const handleLeaderSectionFilter = useCallback(
    (sectionName) => {
  ...
  const handleReportFilterValue = useCallback(
    (value) => {
  ...
  const loadDashboardMetrics = useCallback(async () => {
    ...
    try {
      ...
      // Update todayStats
      const metricsStats = res.data.todayStats ||
        res.data.comparisons?.todayStats || { present: 0, absent: 0, excused: 0 };
  ...
  const handleLeaderSave = useCallback(
    async (data) => {
  ...
  const handleSaveServiceAssignments = useCallback(
    async (leaderIdsArray) => {
      ...
      try {
        ...
        showMessage(
          'Failed to save service assignments. Ensure service exists and date is valid.',
          4000
        );
      } finally {
        setAssignmentsLoading(false);
  ...
  return {
    ...
    setFilterType,
    filterValue,
    setFilterValue: handleReportFilterValue,
    overviewData,
    overviewLoading,
    loadOverview,
    // History
    ...
    handleLeaderDelete,
    leaderSectionFilter,
    setLeaderSectionFilter: handleLeaderSectionFilter,
    memberSectionFilter,
    setMemberSectionFilter: handleMemberSectionFilter,
    memberLeaderFilter,
    setMemberLeaderFilter: handleMemberLeaderFilter,
    // Service Assignments
    isAssignmentModalOpen,
    setIsAssignmentModalOpen,
    selectedInstanceDate,
    setSelectedInstanceDate,
    assignmentsLoading,
    assignedLeaderIds,
