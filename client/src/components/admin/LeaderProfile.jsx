...
const LeaderProfile = ({ leaderId, onBack, allMembers = [] }) => {
  ...
  const exportLeaderPDF = async () => {
    ...
    let weeksList = [];
    try {
      const matrixRes = await analyticsAPI.getMemberWeeklyMatrix({ leaderId, weeks: 12 });
    ...
    if (matrixData.length > 0 && weeksList.length > 0) {
      ...
      autoTableMod.default(doc, {
        ...
        headStyles: { fillColor: [99, 102, 241], textColor: 255, fontStyle: 'bold', halign: 'center', fontSize: 6 },
      });
      y = doc.lastAutoTable.finalY + 12;
    }
