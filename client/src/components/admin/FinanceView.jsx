...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${STATUS_COLORS[status] || ''}`}
    >
...
const FinanceView = ({ showMessage, userRole = 'admin' }) => {
  ...
  const financeMemberContributions = dbData.memberContributions.filter((c) =>
    String(c.reference_number || '').startsWith('finance-')
  );
  const financeContributionTotal = financeMemberContributions.reduce(
    (sum, contribution) => sum + Number(contribution.amount || 0),
    0
  );
  const allContributionTotal = dbData.memberContributions.reduce(
    (sum, contribution) => sum + Number(contribution.amount || 0),
    0
  );
  const financeIncomeTotal = dbData.finance.reduce(
    (sum, record) => sum + Number(record.total_income || 0),
    0
  );
  const financeExpenseTotal = dbData.finance.reduce(
    (sum, record) => sum + Number(record.total_expenses || 0),
    0
  );
  const usableFundsTotal = dbData.finance.reduce(
    (sum, record) => sum + Number(record.usable_church_funds || 0),
    0
  );
  ...
  const exportCSV = () => {
    const rows = [
      ['Date', 'Status', 'Morning', 'Afternoon', 'Tithes', 'Income', 'Expenses', 'Usable']
    ];
    filteredRecords.forEach((r) =>
      rows.push([
        r.record_date,
        r.status,
        r.morning_offering || 0,
        r.afternoon_offering || 0,
        r.auto_tithes ?? r.total_tithes ?? 0,
        r.total_income || 0,
        r.total_expenses || 0,
        r.usable_church_funds || 0
      ])
    );
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {activeTab === 'records' && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            {
              label: 'Today',
              value: todayRecord ? 'Recorded' : 'None',
              icon: DollarSign,
              color: todayRecord ? 'emerald' : 'slate'
            }
          ].map((card) => (
            <div
              key={card.label}
                ...
                <thead className="bg-slate-50 dark:bg-slate-800">
                  <tr className="border-b border-slate-200 dark:border-slate-700">
                    {[
                      ...
                      <th
                        key={h}
                        className={`py-3 px-4 text-[10px] font-semibold uppercase text-slate-400 ${h === '' ? 'text-right' : h === 'Date' ? 'text-left' : 'text-right'}`}
                      >
                        {h}
                      </th>
                    ))}
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td colSpan={9} className="py-16 text-center">
                        ...
                        >
                          Create your first daily record
                        </button>
                      </td>
                    </tr>
                  ) : (
                    filteredRecords.map((r) => (
                      ...
                      >
                        <td className="py-3 px-4 text-left font-medium text-slate-900 dark:text-white">
                          {fdate(r.record_date)}
                        </td>
                        ...
                        <td className="py-3 px-4 text-right text-violet-600">
                          {fmt(r.auto_tithes ?? r.total_tithes)}
                        </td>
                        <td className="py-3 px-4 text-right font-bold text-slate-900 dark:text-white">
                          {fmt(r.total_income)}
                        </td>
                        <td className="py-3 px-4 text-right text-rose-500">
                          {fmt(r.total_expenses || 0)}
                        </td>
                        <td className="py-3 px-4 text-right font-bold text-emerald-600">
                          {fmt(r.usable_church_funds)}
                        </td>
                        <td className="py-3 px-4 text-right">
                          <button
                            onClick={(e) => {
          ...
          {pendingRecords.length === 0 ? (
            ...
            <div className="space-y-2">
              {pendingRecords.map((r) => (
                <div
                  key={r.id}
                  className="rounded-xl bg-white dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700 p-4 hover:shadow-md transition-shadow cursor-pointer"
                  onClick={() => openRecord(r.id)}
                >
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400">
                          by {r.submitted_by_name || 'Unknown'} ·{' '}
                          {r.submitted_at ? fdatetime(r.submitted_at) : ''}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  {
                    label: 'Morning Total',
                    value: fmt(summary.total_morning),
                    icon: ArrowUpCircle
                  },
                  {
                    label: 'Afternoon Total',
                    value: fmt(summary.total_afternoon),
                    icon: ArrowUpCircle
                  },
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  {
                    label: 'Mission Fund (10%)',
                    value: fmt(summary.total_mission),
                    color: 'amber'
                  },
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div className="rounded-xl border border-slate-200/60 dark:border-slate-700 bg-white dark:bg-slate-800 p-4">
                  ...
                  <p className="text-lg font-bold text-emerald-600 mt-1">
                    {fmt(summary.total_usable - summary.total_expenses)}
                  </p>
                </div>
              </div>
            </>
      ...
      {activeTab === 'contributions' && (
        <div className="rounded-2xl border border-slate-200/60 dark:border-slate-700 bg-white dark:bg-slate-900 p-4">
          <ContributionsView allMembers={allMembers} showMessage={showMessage} />
        </div>
      )}
        ...
        (dbLoading ? (
          ...
          <div className="space-y-6">
              ...
              {[
                {
                  ...
                  value: `TZS ${financeContributionTotal.toLocaleString()}`,
                  icon: HandCoins,
                ...
                },
                {
                  ...
                  value: `TZS ${allContributionTotal.toLocaleString()}`,
                  icon: HandCoins,
                ...
                },
                {
                  ...
                  value: `TZS ${financeIncomeTotal.toLocaleString()}`,
                  icon: DollarSign,
                ...
                },
                {
                  ...
                  value: `TZS ${financeExpenseTotal.toLocaleString()}`,
                  icon: AlertCircle,
                ...
                },
                {
                  ...
                  value: `TZS ${(usableFundsTotal - financeExpenseTotal).toLocaleString()}`,
                  icon: Building2,
                ...
                },
                {
                  ...
                  value: dbData.finance.length.toString(),
                  icon: Building2,
                  border: 'border-blue-200/70',
                  darkBorder: 'dark:border-blue-700',
                  textColor: 'text-blue-600',
                ...
                },
                {
                  ...
                  value: new Set(
                    financeMemberContributions.map((c) => c.member_id)
                  ).size.toString(),
                  icon: Users,
                  border: 'border-violet-200/70',
                  darkBorder: 'dark:border-violet-700',
                ...
                {
                  label: 'Total Members',
                  value: dbData.members.toLocaleString(),
                  icon: Users,
                ...
                >
                  ...
                  <p className={`text-2xl font-bold ${card.textColor} ${card.darkText}`}>
                    {card.value}
                  </p>
                </div>
              ))}
            ...
            {(() => {
              ...
              const monthlyData = Object.values(monthly).sort((a, b) =>
              ...
              monthlyData.forEach((d) => {
                ...
                d.name = monthLabels[parts[1]] || parts[1];
              });

              const expensesByCat = {};
              allExpenses.forEach((e) => {
                const cat = e.category || 'Other';
                expensesByCat[cat] = (expensesByCat[cat] || 0) + Number(e.amount);
              });
              const expenseData = Object.entries(expensesByCat).map(([name, total]) => ({
                name,
                total
              }));
              ...
              dbData.memberContributions.forEach((c) => {
                const t = c.contribution_type_name || 'Other';
                contribByType[t] = (contribByType[t] || 0) + Number(c.amount);
              });
              const contribTypeData = Object.entries(contribByType).map(([name, total]) => ({
                name,
                total
              }));
              const CHART_COLORS = [
                      ...
                      <ResponsiveContainer width="100%" height={260}>
                        ...
                        >
                          <defs>
                            ...
                            <linearGradient id="mu" x1="0" y1="0" x2="0" y2="1">
                          ...
                          />
                          <Tooltip
                            formatter={(v) => [`TZS ${Number(v).toLocaleString()}`, undefined]}
                          />
                          <Legend />
                          <Area
                            type="monotone"
                  ...
                  <table className="min-w-[560px] text-sm">
                    <thead>
                      <tr className="border-b border-slate-200 dark:border-slate-700 text-left">
                        ...
                        <th className="py-2 px-2 text-right font-semibold text-slate-600">
                          Income
                        </th>
                        <th className="py-2 px-2 text-right font-semibold text-slate-600">
                          Expenses
                    ...
                    <tbody>
                      {dbData.finance.slice(0, 5).map((r) => (
                        <tr
                          ...
                          onClick={() => openRecord(r.id)}
                        >
                          <td className="py-2.5 px-2 text-slate-900 dark:text-white font-medium">
                            {fdate(r.record_date)}
                          </td>
                          ...
                          <td className="py-2.5 px-2 text-right font-semibold text-emerald-600">
                            TZS {Number(r.total_income || 0).toLocaleString()}
                          </td>
                          <td className="py-2.5 px-2 text-right text-rose-500">
                            TZS {Number(r.total_expenses || 0).toLocaleString()}
                          </td>
                          <td className="py-2.5 px-2 text-right font-bold text-indigo-600 dark:text-indigo-400">
                            TZS {Number(r.usable_church_funds || 0).toLocaleString()}
                          </td>
                        </tr>
                      ))}
                      {dbData.finance.length === 0 && (
                        <tr>
                          <td colSpan={5} className="text-center py-8 text-slate-400">
                            No daily records yet this month
                          </td>
                        </tr>
                      )}
                    </tbody>
                    ...
                    <tbody>
                      {allExpenses.slice(0, 5).map((e) => (
                        <tr key={e.id} className="border-b border-slate-100 dark:border-slate-700">
                          ...
                          <td className="py-2.5 px-2">
                            <span className="text-xs px-1.5 py-0.5 rounded-full bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400 font-semibold">
                              {e.category}
                            </span>
                          </td>
                          <td
                            className="py-2.5 px-2 text-slate-600 dark:text-slate-300 max-w-[150px] truncate"
                            title={e.description}
                          >
                            {e.description || '-'}
                          </td>
