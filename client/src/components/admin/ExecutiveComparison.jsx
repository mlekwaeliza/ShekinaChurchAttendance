...
const Badge = ({ children, variant = 'neutral' }) => (
  <span
    className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${CHIP[variant] || CHIP.neutral}`}
  >
    {children}
  </span>
);
...
const KpiCard = ({ label, value, prev, diff, pctChg, status, icon: Icon }) => {
  ...
  return (
    <div className="rounded-2xl border border-slate-200/70 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 shadow-sm hover:shadow-md transition-shadow">
      ...
      <p className={`text-xl font-bold text-${statusColor}-600 dark:text-${statusColor}-400`}>
        {value}
      </p>
      <div className="flex items-center gap-2 mt-1.5 text-[10px]">
        {diff !== undefined && (
          <span
            className={`flex items-center gap-0.5 font-semibold ${diff >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
          >
...
const ExecutiveComparison = () => {
  ...
  useEffect(() => {
    ...
    const loadMatrix = async () => {
      ...
      try {
        ...
        console.error('Failed to load member matrix:', e);
        setMemberMatrix([]);
      } finally {
  ...
  const trendInsights = useMemo(() => {
    ...
    if (trends.classification) {
      ...
      const type =
        cls === 'growing' || cls === 'recovering'
          ? 'success'
          : cls === 'declining' || cls === 'volatile'
            ? 'danger'
            : 'info';
      const label =
        cls === 'growing'
      ...
      list.push({
        type,
        text: `${label}. Direction: ${trends.direction != null ? (trends.direction >= 0 ? '+' : '') + trends.direction + '%' : 'N/A'} (${trends.first_period_rate || 0}% → ${trends.last_period_rate || 0}%).`
      });
    }
    if (trends.recent_momentum != null) {
      list.push({
        type: trends.recent_momentum >= 0 ? 'success' : 'warning',
        text: `Recent momentum: ${trends.recent_momentum >= 0 ? '+' : ''}${trends.recent_momentum}% over last 3 periods.`
      });
    }
    asArray(trends.anomalies).forEach((a) => {
      list.push({
        type: a.type === 'positive' ? 'success' : 'danger',
        text: `Anomaly in ${a.period}: ${a.rate}% (${a.deviation > 0 ? '+' : ''}${a.deviation}% deviation).`
      });
    });
    if (trends.average_rate != null) {
      list.push({
        type: 'info',
        text: `Average attendance rate across periods: ${trends.average_rate}%.`
      });
    }
    return list;
  }, [trends]);
  ...
  const ratePct = prevOverall.attendance_rate
    ? R((rateDiff / prevOverall.attendance_rate) * 100)
    : 0;
        ...
        <>
          ...
          {view === 'summary' && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-3">
                <KpiCard
                  label="Church Health"
                  value={`${kpis?.church_health_score || 0}%`}
                  diff={kpis?.rate_change}
                  status="above_target"
                  icon={Heart}
                />
                <KpiCard
                  label="Attendance Rate"
                  value={pct(latestOverall.attendance_rate)}
                  prev={pct(prevOverall.attendance_rate)}
                  diff={rateDiff}
                  pctChg={pct(ratePct)}
                  status={rateDiff >= 0 ? 'above_target' : 'below_target'}
                  icon={Activity}
                />
                <KpiCard
                  label="Growth Index"
                  value={kpis?.attendance_growth_index || 0}
                  status={kpis?.attendance_growth_index >= 0 ? 'above_target' : 'below_target'}
                  icon={TrendingUp}
                />
                <KpiCard
                  label="Retention"
                  value={pct(kpis?.retention_index)}
                  status={
                    kpis?.retention_index >= 70
                      ? 'above_target'
                      : kpis?.retention_index >= 50
                        ? 'on_target'
                        : 'below_target'
                  }
                  icon={UserCheck}
                ...
                <KpiCard
                  ...
                  value={`${latestOverall.leaders_submitted || 0}/${latestOverall.active_sections || 0}`}
                  status={
                    (latestOverall.leaders_submitted || 0) >= (latestOverall.active_sections || 1)
                      ? 'above_target'
                      : 'below_target'
                  }
                  icon={Award}
                ...
                <KpiCard
                  ...
                  value={kpis?.followup_count || latestOverall.absent || 0}
                  status={
                    (kpis?.followup_count || latestOverall.absent || 0) > 20
                      ? 'below_target'
                      : 'on_target'
                  }
                  icon={AlertTriangle}
                  ...
                  <table className="w-full text-xs">
                    <thead>
                      <tr className="border-b border-slate-200 dark:border-slate-700">
                        ...
                        <th className="text-right py-2 px-3 font-semibold text-slate-500">
                          Current
                        </th>
                        <th className="text-right py-2 px-3 font-semibold text-slate-500">
                          Previous
                    ...
                    <tbody>
                      {[
                        ...
                        const isGoodTrend =
                          positive === 'up' ? current >= previous : current <= previous;
                        return (
                          <tr
                            key={row.key}
                          ...
                          >
                            ...
                            <td
                              className={`py-2.5 px-3 text-right font-semibold ${diff >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                            >
                              {diff >= 0 ? '+' : ''}
                              {row.fmt(diff)}
                            </td>
                            <td className="py-2.5 px-3 text-center">
                              {current !== previous ? (
                                <span
                                  className={`inline-flex items-center justify-center ${isGoodTrend ? 'text-emerald-500' : 'text-rose-500'}`}
                                >
                                  {current > previous ? (
                    ...
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-white dark:bg-slate-800">
                        <tr className="border-b border-slate-200 dark:border-slate-700">
                          ...
                          <th
                            className="text-left py-2 px-2 font-semibold text-slate-500 cursor-pointer"
                          ...
                          {dataPeriods.map((p) => (
                            <th
                              key={p.id}
                              className="text-right py-2 px-2 font-semibold text-slate-500"
                            >
                              {p.label}
                            </th>
                          ))}
                          <th
                            className="text-right py-2 px-2 font-semibold text-slate-500 cursor-pointer"
                            onClick={() => activeSort('attendance_rate')}
                          >
                            Avg Rate
                          </th>
                      ...
                      <tbody>
                        {(mode === 'sections'
                          ? sortFn(dataPeriods[dataPeriods.length - 1]?.sections, 'attendance_rate')
                          : mode === 'leaders'
                            ? sortFn(
                                dataPeriods[dataPeriods.length - 1]?.leaders,
                                'attendance_rate'
                              )
                            : mode === 'departments'
                              ? sortFn(
                                  dataPeriods[dataPeriods.length - 1]?.departments,
                                  'attendance_rate'
                                )
                              : sortFn(
                                  dataPeriods[dataPeriods.length - 1]?.memberEngagement,
                                  'attendance_rate'
                                )
                        )
                          .filter(
                            (e) =>
                              !search ||
                              (e.name || e.leader_name || e.full_name || '')
                                .toLowerCase()
                          ...
                          .map((entity, i) => {
                            const rates = dataPeriods.map((p) => {
                              ...
                              const found = list?.find(
                                (e2) =>
                                  e2.id === entity.id ||
                                  e2.name === entity.name ||
                                  e2.leader_name === entity.leader_name
                              );
                            ...
                            const trend =
                              rates.length > 1
                                ...
                                : 'stable';
                            const name =
                              entity.name || entity.leader_name || entity.full_name || 'Unknown';
                            return (
                              <tr
                                key={entity.id || i}
                                className="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/30"
                              >
                                ...
                                {rates.map((r, j) => (
                                  <td
                                    key={j}
                                    className="py-2 px-2 text-right font-semibold text-slate-900 dark:text-white"
                                  >
                                    {pct(r)}
                                  </td>
                                ))}
                ...
                <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
                  <table className="w-full text-xs">
                    <thead className="sticky top-0 bg-white dark:bg-slate-800 z-10">
                      <tr className="border-b border-slate-200 dark:border-slate-700">
                        <th className="text-left py-2 px-2 font-semibold text-slate-500 sticky left-0 bg-white dark:bg-slate-800 z-20">
                          Metric
                        </th>
                        {dataPeriods.map((p) => (
                          <th
                            key={p.id}
                            className="text-right py-2 px-3 font-semibold text-slate-500 min-w-[90px]"
                          >
                            {p.label}
                          </th>
                        ))}
                        <th className="text-right py-2 px-3 font-semibold text-slate-500 min-w-[70px]">
                          Avg
                        </th>
                        <th className="text-right py-2 px-3 font-semibold text-slate-500 min-w-[70px]">
                          Best
                      ...
                      {[
                        ...
                        return (
                          <tr
                            ...
                            className="border-b border-slate-100 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/30"
                          >
                            <td className="py-1.5 px-2 font-medium text-slate-700 dark:text-slate-300 sticky left-0 bg-white dark:bg-slate-800">
                              {row.label}
                            </td>
                            {vals.map((v, i) => (
                              <td
                                ...
                                className={`py-1.5 px-3 text-right font-semibold ${
                                  Number(v) === best && nums.some((n) => n !== best)
                                    ? 'text-emerald-600'
                                    : Number(v) === worst && nums.some((n) => n !== worst)
                                      ? 'text-rose-600'
                            ...
                            <td className="py-1.5 px-3 text-right font-bold text-rose-600">
                              {row.fmt(worst)}
                            </td>
                          </tr>
                        );
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                {['best', 'worst', 'improving', 'declining'].map((type) => {
                  ...
                  const colors = {
                    best: {
                      bg: 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-200 dark:border-emerald-700',
                      icon: 'text-emerald-500',
                      label: 'Best Performing'
                    },
                    worst: {
                      bg: 'bg-rose-50 dark:bg-rose-900/10 border-rose-200 dark:border-rose-700',
                      icon: 'text-rose-500',
                      label: 'Needs Attention'
                    },
                    improving: {
                      bg: 'bg-blue-50 dark:bg-blue-900/10 border-blue-200 dark:border-blue-700',
                      icon: 'text-blue-500',
                      label: 'Most Improved'
                    },
                    declining: {
                      bg: 'bg-amber-50 dark:bg-amber-900/10 border-amber-200 dark:border-amber-700',
                      icon: 'text-amber-500',
                      label: 'Declining'
                    }
                  };
                    ...
                    <div key={type} className={`rounded-2xl border ${c.bg} p-4 shadow-sm`}>
                      <h4 className="text-xs font-semibold uppercase text-slate-500 mb-3 flex items-center gap-1.5">
                        {type === 'best' ? (
                          ...
                          <TrendingUp className={`w-3.5 h-3.5 ${c.icon}`} />
                        ) : (
                  ...
                  <table className="w-full text-xs">
                    <thead className="sticky top-0 bg-white dark:bg-slate-800">
                      <tr className="border-b border-slate-200 dark:border-slate-700">
                        ...
                        <th
                          className="text-right py-2 px-2 font-semibold text-slate-500 cursor-pointer"
                          onClick={() => activeSort('submission_rate')}
                        >
                          Submit %
                        </th>
                        <th
                          className="text-right py-2 px-2 font-semibold text-slate-500 cursor-pointer"
                          onClick={() => activeSort('follow_up_completion')}
                        >
                          Follow-up
                        </th>
                          ...
                          return (
                            <tr
                              ...
                              onClick={() => {
                                isSection &&
                                  setExpandedSection(
                                    expandedSection === entity.id ? null : entity.id
                                  );
                              }}
                            >
                              ...
                              {mode === 'members' ? (
                                <td className="py-2 px-2 text-right whitespace-nowrap">
                                  <span className="text-emerald-600 font-bold">
                                    {entity.present_count || 0}
                                  </span>
                                  <span className="text-slate-400 mx-0.5">/</span>
                                  <span className="text-rose-600 font-bold">
                                    {entity.absent_count || 0}
                                  </span>
                                  <span className="text-slate-400 mx-0.5">/</span>
                                  <span className="text-amber-600 font-bold">
                                    {entity.excused_count || 0}
                                  </span>
                                </td>
                              ) : (
                                <td className="py-2 px-2 text-right font-bold text-indigo-600">
                                  {pct(entity.attendance_rate)}
                                </td>
                              )}
                              <td className="py-2 px-2 text-right text-slate-900 dark:text-white">
                                {entity.members || entity.assigned_members || '-'}
                              </td>
                              {mode !== 'members' && (
                                <td className="py-2 px-2 text-right text-slate-900 dark:text-white">
                                  {entity.present || '-'}
                              ...
                              <td className="py-2 px-2 text-right">
                                {entity.submission_rate !== undefined ? (
                                  <span
                                    className={`font-semibold ${entity.submission_rate >= 80 ? 'text-emerald-600' : entity.submission_rate >= 50 ? 'text-amber-600' : 'text-rose-600'}`}
                                  >
                              ...
                              <td className="py-2 px-2 text-right">
                                {entity.follow_up_completion !== undefined ? (
                                  <span
                                    className={`font-semibold ${entity.follow_up_completion >= 70 ? 'text-emerald-600' : entity.follow_up_completion >= 40 ? 'text-amber-600' : 'text-rose-600'}`}
                                  >
                                ...
                                {(() => {
                                  const prevEntity =
                                    dataPeriods.length > 1
                                      ? dataPeriods[dataPeriods.length - 2]?.[
                                          mode === 'sections' ? 'sections' : 'leaders'
                                        ]?.find((e2) => e2.id === entity.id)
                                      : null;
                                  const diff = prevEntity
                                    ? (entity.attendance_rate || 0) -
                                      (prevEntity.attendance_rate || 0)
                                    : 0;
                                  return diff > 2 ? (
                                    <Badge variant="success">↑{R(diff)}</Badge>
                                  ) : diff < -2 ? (
                                    <Badge variant="danger">↓{Math.abs(R(diff))}</Badge>
                                  ) : (
            ...
            <div className="rounded-2xl border border-slate-200/70 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm">
              <h3 className="font-semibold text-slate-900 dark:text-white mb-4 flex items-center gap-2">
                ...
                <span className="text-[10px] font-normal text-slate-400 ml-1">
                  (per week: present / absent / excused)
                </span>
              </h3>
              {memberMatrixLoading ? (
                ...
                <div className="overflow-x-auto max-h-[600px] overflow-y-auto">
                  <div className="min-w-max">
                    <div className="flex bg-slate-50 dark:bg-slate-900/40 border-b border-slate-200 dark:border-slate-700 text-[10px] font-bold uppercase text-slate-500">
                      <div className="sticky left-0 z-10 bg-slate-50 dark:bg-slate-900/40 w-44 shrink-0 px-3 py-2">
                        Member
                      </div>
                      <div className="w-28 shrink-0 px-3 py-2">Section</div>
                      {memberMatrixWeeks.map((w) => (
                        <div key={w} className="w-20 shrink-0 px-2 py-2 text-center">
                          {weekToDate(w)}
                        </div>
                      ))}
                    </div>
                    {memberMatrix
                      ...
                      .map((m) => (
                        ...
                        >
                          <div className="sticky left-0 z-10 bg-white dark:bg-slate-800 w-44 shrink-0 px-3 py-2.5 font-semibold text-slate-900 dark:text-white truncate">
                            {m.full_name}
                          </div>
                          <div className="w-28 shrink-0 px-3 py-2.5 text-slate-500 truncate">
                            {m.section_name || '—'}
                          </div>
                          {asArray(m.weekly).map((status, wi) => (
                            <div key={wi} className="w-20 shrink-0 px-2 py-2 text-center">
                              {status === 'present' && (
                                <span className="inline-flex items-center gap-1 text-emerald-700 bg-emerald-50 dark:bg-emerald-900/30 dark:text-emerald-200 px-2 py-1 rounded-full text-[10px] font-semibold">
                                  <CheckCircle2 className="w-3 h-3" />
                                  Present
                                </span>
                              )}
                              {status === 'absent' && (
                                <span className="inline-flex items-center gap-1 text-rose-700 bg-rose-50 dark:bg-rose-900/30 dark:text-rose-200 px-2 py-1 rounded-full text-[10px] font-semibold">
                                  <XCircle className="w-3 h-3" />
                                  Absent
                                </span>
                              )}
                              {status === 'excused' && (
                                <span className="inline-flex items-center gap-1 text-amber-700 bg-amber-50 dark:bg-amber-900/30 dark:text-amber-200 px-2 py-1 rounded-full text-[10px] font-semibold">
              ...
              <div className="rounded-2xl border border-slate-200/70 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                    ...
                    <p className="text-lg font-bold mt-1 flex items-center gap-2">
                      {trends?.classification === 'growing' ? (
                        ...
                        <span className="text-slate-600">
                          {trends?.classification || 'Analyzing...'}
                        </span>
                      )}
                    </p>
                  </div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-700/50 rounded-xl p-3">
                    ...
                    <p
                      className={`text-lg font-bold mt-1 ${(trends?.recent_momentum || 0) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                    >
                      {trends?.recent_momentum > 0 ? '+' : ''}
                      {R(trends?.recent_momentum || 0)}%
                    </p>
                  </div>
                </div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="space-y-1.5">
                      {asArray(trends?.anomalies).map((a, i) => (
                        <div
                          key={i}
                          className="flex items-center gap-3 text-xs bg-amber-50 dark:bg-amber-900/10 rounded-lg px-3 py-2"
                        >
                          <Badge variant={a.type === 'positive' ? 'success' : 'danger'}>
                            {a.deviation > 0 ? '+' : ''}
                            {a.deviation}%
                          </Badge>
                          ...
                          <span className="text-slate-400 ml-auto">
                            {a.type === 'positive' ? 'Above average' : 'Below average'}
                          </span>
                        </div>
                      ))}
                    </div>
                ...
                <div className="rounded-2xl border border-slate-200/70 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm">
                  ...
                  {asArray(rootCauses?.factors).length > 0 && (
                    <div className="space-y-2">
                      {asArray(rootCauses?.factors)
                        .slice(0, 10)
                        .map((f, i) => (
                          <div
                            key={i}
                            className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-700/30 rounded-lg px-3 py-2"
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              <Badge variant={f.type === 'section' ? 'info' : 'violet'}>
                                {f.type}
                              </Badge>
                            ...
                            <div className="flex items-center gap-3">
                              <span
                                className={`font-bold ${f.direction === 'positive' ? 'text-emerald-600' : 'text-rose-600'}`}
                              >
                                {f.impact > 0 ? '+' : ''}
                                {f.impact}%
                              </span>
                              <span className="text-slate-400">
                                ({Math.abs(f.contribution)}% contrib.)
                              </span>
                            </div>
                          </div>
                        ))}
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-700/30 rounded-lg px-3 py-2">
                      ...
                      <span className="font-bold text-emerald-600">
                        {latestOverall.new_members || 0}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-700/30 rounded-lg px-3 py-2">
                      <span className="text-slate-600 dark:text-slate-400">Returning Members</span>
                      <span className="font-bold text-blue-600">
                        {latestOverall.returning_members ||
                          latestPeriod?.movement?.returning_members ||
                          0}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-700/30 rounded-lg px-3 py-2">
                      ...
                      <span className="font-bold text-indigo-600">
                        {latestPeriod?.movement?.visitors_converted || 0}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-700/30 rounded-lg px-3 py-2">
                      <span className="text-slate-600 dark:text-slate-400">Members Lost</span>
                      <span className="font-bold text-rose-600">
                        {latestOverall.members_lost || latestPeriod?.movement?.members_lost || 0}
                      </span>
                    </div>
                    <div className="flex items-center justify-between text-xs bg-slate-50 dark:bg-slate-700/30 rounded-lg px-3 py-2 border-t border-slate-200 dark:border-slate-700 mt-2 pt-2">
                      ...
                      <span
                        className={`font-bold text-lg ${(latestOverall.net_growth || 0) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                      >
                        {(latestOverall.net_growth || 0) >= 0 ? '+' : ''}
                        {latestOverall.net_growth || 0}
                      </span>
                    </div>
                  </div>
                </div>
                  ...
                  <table className="w-full text-xs">
                    ...
                    <tbody>
                      {dataPeriods.map((p, i) => {
                        ...
                        const rateChange = prev
                          ? (o.attendance_rate || 0) - (prev.attendance_rate || 0)
                          : 0;
                        const isBest =
                          i === 0 ||
                          (o.attendance_rate || 0) >=
                            Math.max(
                              ...dataPeriods
                                .slice(0, i + 1)
                                .map((x) => x.overall?.attendance_rate || 0)
                            );
                        const isWorst =
                          i === 0 ||
                          (o.attendance_rate || 0) <=
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-2 px-2 text-right">
                              <span
                                className={`font-bold ${rateChange > 0 ? 'text-emerald-600' : rateChange < 0 ? 'text-rose-600' : 'text-slate-400'}`}
                              >
                                {rateChange > 0 ? '+' : ''}
                                {R(rateChange)}%
                              </span>
          ...
          {view === 'actions' && (
            <div className="space-y-6">
              ...
              {asArray(latestPeriod?.memberEngagement).filter(
                          ...
                          .map((m) => (
                            ...
                            >
                              ...
                              <td className="py-2 px-2 text-right text-slate-500">
                                {m.last_attendance === 'never' ? 'Never' : fdate(m.last_attendance)}
                              </td>
                              <td className="py-2 px-2 text-right">
                                <Badge variant={m.risk_level === 'critical' ? 'danger' : 'warning'}>
                                  {m.risk_level}
                                </Badge>
                              </td>
                            </tr>
