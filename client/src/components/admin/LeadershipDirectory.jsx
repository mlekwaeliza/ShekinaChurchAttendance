...
const LeadershipDirectory = () => {
  ...
  const handleExport = () => {
    ...
    const rows = data.directory.map((d) => [
      ...
      d.appointment_date ? fdate(d.appointment_date) : '—',
      d.phone || '—',
      d.email || '—',
      d.assigned_by_name || '—',
    ]);
  ...
  const columns = [
    ...
    { accessor: 'appointment_date', header: 'Appointed', sortable: true, render: (row) => row.appointment_date ? fdate(row.appointment_date) : '—' },
