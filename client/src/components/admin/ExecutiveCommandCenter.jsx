...
const Row = ({ to, navigate, children, className = '', ...rest }) => {
  ...
  return (
    <div
      className={`flex items-center gap-3 p-2 rounded-xl bg-slate-50 dark:bg-slate-700/30 ${className}`}
      {...rest}
    >
      {children}
    </div>
...
const Metric = ({ label, value, sub, status, trend }) => (
  <div className="flex flex-col">
    ...
    {trend != null && trend !== 0 && (
      <span
        className={`text-[10px] font-semibold mt-0.5 ${trend > 0 ? 'text-emerald-600' : 'text-rose-600'}`}
      >
...
const ExecutiveCommandCenter = (props) => {
  ...
  const comparison = comparisonProp;
  const aiInsights = aiInsightsProp;
  const homeCells = homeCellsProp;
  const departments = departmentsProp;
  const audit = auditProp;
  const hallOfFame = hallOfFameProp;
  const backup = backupProp;
  const health = healthProp;
  const notifCount = notifCountProp;

  const alerts = asArray(summary?.alerts);
  ...
  const scoreStatus =
    churchScore?.current >= 85
      ? 'excellent'
      : churchScore?.current >= 70
        ? 'good'
        : churchScore?.current >= 50
          ? 'watch'
          : churchScore?.current >= 30
            ? 'attention'
            : 'critical';
  ...
  const compRates = asArray(comparison?.periods)
    .map((p) => Number(p?.overall?.attendance_rate) || 0)
    .filter((r) => r > 0);
  ...
  const deptMemberTotal = asArray(departments).reduce(
    (sum, d) => sum + (Number(d.member_count) || Number(d.memberCount) || 0),
    0
  );
  ...
  const pulse = [
    {
      icon: Activity,
      label: 'Attendance',
      value: pct(kpis.attendanceRate?.current),
      status: statusForScore(kpis.attendanceRate?.current || 0),
      to: '/admin/reports'
    },
    {
      icon: ShieldCheck,
      label: 'Leadership',
      value: pct(kpis.leaderPerfIndex?.current),
      status: statusForScore(kpis.leaderPerfIndex?.current || 0),
      to: '/admin/leaders'
    },
    {
      icon: Layers,
      label: 'Sections',
      value: pct(kpis.sectionPerfIndex?.current),
      status: statusForScore(kpis.sectionPerfIndex?.current || 0),
      to: '/admin/sections'
    },
    {
      icon: UserCheck,
      label: 'Retention',
      value: pct(kpis.retentionRate?.current),
      status: statusForScore(kpis.retentionRate?.current || 0),
      to: '/admin/analytics'
    },
    {
      icon: Send,
      label: 'Visitor FU',
      value: pct(kpis.visitorConversion?.current),
      status: statusForScore(kpis.visitorConversion?.current || 0),
      to: '/admin/follow-ups'
    },
    {
      icon: Sparkles,
      label: 'Engagement',
      value: pct(kpis.engagementScore?.current),
      status: statusForScore(kpis.engagementScore?.current || 0),
      to: '/admin/analytics'
    },
    {
      icon: Home,
      label: 'Home Cells',
      value: homeCells.length ? `${homeCells.length}` : '—',
      status: homeCells.length ? 'good' : 'neutral',
      to: '/admin/home-cells'
    },
    {
      icon: Building2,
      label: 'Ministry',
      value: departments.length ? `${departments.length}` : '—',
      status: departments.length ? 'good' : 'neutral',
      to: '/admin/departments'
    },
    {
      icon: Gift,
      label: 'Contrib.',
      value: kpis.contributionsIndex?.current != null ? pct(kpis.contributionsIndex.current) : '—',
      status:
        kpis.contributionsIndex?.current != null
          ? statusForScore(kpis.contributionsIndex.current)
          : 'neutral',
    ...
    <div className="space-y-5 animate-fade-in pb-12">
            ...
            <div>
              ...
              <div className="flex items-baseline gap-2">
                <p className="text-4xl font-black text-slate-900 dark:text-white">
                  {churchScore ? R(churchScore.current) : '—'}
                  <span className="text-lg font-normal text-slate-400">/100</span>
                </p>
                {churchScore?.diff != null && (
                  <span
                    className={`text-sm font-semibold ${churchScore.diff >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                  >
                    {churchScore.diff >= 0 ? '+' : ''}
                    {R(churchScore.diff)} vs prev
                  </span>
                )}
              </div>
              <div className="mt-1">
                <StatusPill status={scoreStatus} />
              </div>
            </div>
          </div>
          <div className="flex items-center gap-5 flex-wrap">
            <div className="text-center">
              <p className="text-xl font-bold text-slate-900 dark:text-white">
                {num(snap.church?.totalMembers || allMembers.length)}
              </p>
              <p className="text-[9px] text-slate-400 uppercase">Members</p>
            </div>
            <div className="text-center">
              <p className="text-xl font-bold text-slate-900 dark:text-white">
                {num(snap.church?.activeSections || sections.length)}
              </p>
              <p className="text-[9px] text-slate-400 uppercase">Sections</p>
            </div>
            <div className="text-center">
              <p className="text-xl font-bold text-slate-900 dark:text-white">
                {num(snap.church?.activeLeaders || leaders.length)}
              </p>
              <p className="text-[9px] text-slate-400 uppercase">Leaders</p>
            </div>
            <div className="text-center">
              <p className="text-xl font-bold text-slate-900 dark:text-white">
                {pct(kpis.attendanceRate?.current)}
              </p>
            ...
            {churchScore?.target != null && (
              <div className="text-center">
                <p className="text-xl font-bold text-slate-900 dark:text-white">
                  {R(churchScore.target)}
                </p>
                <p className="text-[9px] text-slate-400 uppercase">Target</p>
              </div>
            )}
      ...
      <div className="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700 shadow-sm p-3">
        <div className="flex items-center gap-2 overflow-x-auto">
          {pulse.map((p, i) => (
            <PulseItem key={i} {...p} navigate={navigate} />
          ))}
        </div>
      </div>
      ...
      <SectionCard
        title="Critical Alerts & Needs Attention"
        icon={AlertTriangle}
        to="/admin/follow-ups"
        navigate={navigate}
        action={<span className="text-[10px] font-bold text-rose-600">{alerts.length} items</span>}
      >
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          <Metric
            label="Total Members"
            value={num(snap.attendance?.totalEligible || allMembers.length)}
          />
          ...
          <Metric
            label="Difference"
            value={
              kpis.attendanceRate?.diff != null
                ? `${kpis.attendanceRate.diff >= 0 ? '+' : ''}${R(kpis.attendanceRate.diff)}%`
                : INSUFFICIENT
            }
          />
          <Metric
            label="Target"
            value={churchScore?.target != null ? R(churchScore.target) : INSUFFICIENT}
          />
          <Metric
            label="Best Period"
            value={compBest != null ? pct(compBest) : INSUFFICIENT}
            status="excellent"
          />
          <Metric
            label="Worst Period"
            value={compWorst != null ? pct(compWorst) : INSUFFICIENT}
            status="attention"
          />
          <Metric
            label="Weekly Avg"
            value={
              kpis.weeklyGrowth?.current != null ? pct(kpis.weeklyGrowth.current) : INSUFFICIENT
            }
          />
          <Metric
            label="Monthly Avg"
            value={
              kpis.monthlyGrowth?.current != null ? pct(kpis.monthlyGrowth.current) : INSUFFICIENT
            }
          />
          <Metric
            label="Quarterly Avg"
            value={
              kpis.quarterlyGrowth?.current != null
                ? pct(kpis.quarterlyGrowth.current)
                : INSUFFICIENT
            }
          />
          <Metric
            label="Yearly Avg"
            value={
              kpis.yearlyGrowth?.current != null ? pct(kpis.yearlyGrowth.current) : INSUFFICIENT
            }
          />
          <Metric
            label="Historical Avg"
            value={
              kpis.attendanceRate?.historicalAvg != null
                ? pct(kpis.attendanceRate.historicalAvg)
                : INSUFFICIENT
            }
          />
          <Metric
            label="Best Ever"
            value={kpis.attendanceRate?.best != null ? pct(kpis.attendanceRate.best) : INSUFFICIENT}
            status="excellent"
          />
          <Metric
            label="Trend"
            value={comparison?.trends?.trend ? comparison.trends.trend : INSUFFICIENT}
          />
          <Metric
            label="Momentum"
            value={
              kpis.attendanceMomentum?.current != null
                ? pct(kpis.attendanceMomentum.current)
                : INSUFFICIENT
            }
          />
          <Metric
            ...
            value={
              snap.attendance?.serviceDays ? `${snap.attendance.serviceDays} days` : INSUFFICIENT
            }
          />
          <Metric
            label="Avg per Service"
            value={
              snap.attendance?.avgPerService != null
                ? num(snap.attendance.avgPerService)
                : INSUFFICIENT
            }
          />
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        >
          <div className="space-y-2">
            {asArray(summary?.leaderRankings)
              .slice(0, 6)
              .map((l, i) => (
                <Row
                  key={l.id || i}
                  to={`/admin/leaders?profile=${l.id || l.leader_id}`}
                  navigate={navigate}
                >
                  <span
                    className={`w-6 h-6 rounded-lg text-[10px] font-bold flex items-center justify-center ${i === 0 ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500 dark:bg-slate-700'}`}
                  >
                    {i + 1}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-slate-900 dark:text-white truncate">
                      {l.name || l.leader_name}
                    </p>
                    <p className="text-[10px] text-slate-400">
                      {l.section || l.section_name} · {l.members || l.assigned_members} members
                    </p>
                  </div>
                  <div className="text-right">
                    <p
                      ...
                      style={{
                        color:
                          (l.attendance_rate || l.attendanceRate) >= 75
                            ? '#10b981'
                            : (l.attendance_rate || l.attendanceRate) >= 50
                              ? '#f59e0b'
                              : '#ef4444'
                      }}
                    >
                      {R(l.attendance_rate || l.attendanceRate)}%
                    </p>
                    <p className="text-[9px] text-slate-400">
                      sub {R(l.submissionRate ?? l.leader_submission_rate ?? 0)}%
                    </p>
                  </div>
                </Row>
              ))}
        ...
        >
          <div className="space-y-2">
            {asArray(summary?.sectionRankings)
              .slice(0, 6)
              .map((s, i) => {
                const st =
                  s.status === 'strong'
                ...
                return (
                  <Row key={s.id || i} to={`/admin/sections?profile=${s.id}`} navigate={navigate}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-slate-400">
                        {s.members} members · {s.present} present
                      </p>
                    </div>
                    <div className="text-right">
                      <p
      ...
      <SectionCard title="Member Intelligence" icon={Users} to="/admin/members" navigate={navigate}>
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          <Metric
            ...
            value={num(
              snap.church?.activeMembers || allMembers.filter((m) => m.is_active !== 0).length
            )}
            status="good"
          />
          <Metric
            label="Inactive Members"
            value={num(allMembers.filter((m) => m.is_active === 0).length)}
            status="attention"
          />
          <Metric label="Visitors" value={num(snap.church?.visitors)} />
          <Metric
            label="Visitors Converted"
            value={num(snap.church?.visitorsConverted)}
            status="good"
          />
          <Metric
            label="In Departments"
            value={num(deptMemberTotal)}
            status={deptMemberTotal > 0 ? 'good' : 'neutral'}
          />
          <Metric
            label="Not Serving"
            value={num(
          ...
          <Metric
            ...
            status={highPriorityCount > 0 ? 'critical' : 'good'}
          />
          <Metric
            label="Medium Priority Alerts"
            value={num(mediumPriorityCount)}
            status={mediumPriorityCount > 0 ? 'attention' : 'good'}
          />
          {(() => {
            const male = allMembers.filter((m) =>
              (m.gender || '').toLowerCase().startsWith('m')
            ).length;
            const female = allMembers.filter((m) =>
          ...
          <Metric
            label="Youth"
            value={num(
              allMembers.filter((m) => (m.age_group || '').toLowerCase().includes('youth')).length
            )}
          />
          <Metric
            label="Adults"
            value={num(
              allMembers.filter((m) => (m.age_group || '').toLowerCase().includes('adult')).length
            )}
          />
          <Metric
            label="Children"
            value={num(
              allMembers.filter((m) => (m.age_group || '').toLowerCase().includes('child')).length
            )}
          />
          <Metric
            label="Attendance Freq"
            value={pct(kpis.attendanceRate?.current)}
            sub="proxy: att rate"
            status={statusForScore(kpis.attendanceRate?.current || 0)}
          />
      ...
      >
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          <Metric
            label="Weekly Growth"
            value={
              kpis.weeklyGrowth?.current != null
                ? `${kpis.weeklyGrowth.current >= 0 ? '+' : ''}${R(kpis.weeklyGrowth.current)}%`
                : INSUFFICIENT
            }
            status={kpis.weeklyGrowth?.current >= 0 ? 'good' : 'attention'}
          />
          <Metric label="New This Month" value={num(snap.church?.newMembers)} status="good" />
          <Metric
            label="Yearly Growth"
            value={
              kpis.yearlyGrowth?.current != null
                ? `${kpis.yearlyGrowth.current >= 0 ? '+' : ''}${R(kpis.yearlyGrowth.current)}%`
                : INSUFFICIENT
            }
            status={kpis.yearlyGrowth?.current >= 0 ? 'good' : 'attention'}
          />
          <Metric
            label="Returning Members"
            value={num(kpis.returningMembers?.current)}
            status="good"
          />
          ...
          <Metric
            label="Net Growth"
            value={num(snap.church?.netGrowth)}
            status={snap.church?.netGrowth >= 0 ? 'good' : 'critical'}
          />
          <Metric
            label="Retention %"
            value={pct(kpis.retentionRate?.current)}
          ...
          <Metric
            ...
            value={pct(kpis.followUpCompletion?.current)}
            status={statusForScore(kpis.followUpCompletion?.current || 0)}
          />
          <Metric
            label="Ministry Health"
            value={
              kpis.ministryHealth?.current != null ? R(kpis.ministryHealth.current) : INSUFFICIENT
            }
            status={statusForScore(kpis.ministryHealth?.current || 0)}
          />
          <Metric
            label="Goal Achiev."
            value={pct(kpis.goalAchievement?.current)}
            status={statusForScore(kpis.goalAchievement?.current || 0)}
          />
        </div>
      </SectionCard>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <SectionCard
          title="Home Cell Intelligence"
          icon={Home}
          action={
            <button
              onClick={() => navigate('/admin/home-cells')}
              className="text-[10px] text-blue-600 font-semibold"
            >
              View all
            </button>
            ...
            {homeCells.slice(0, 5).map((c, i) => {
              ...
              return (
                <Row key={c.id || i} to="/admin/home-cells" navigate={navigate}>
                  ...
                  <StatusPill
                    status={members > 10 ? 'good' : members > 5 ? 'watch' : 'attention'}
                  />
                </Row>
              );
            })}
              ...
              <Row key={d.id || i} to="/admin/departments" navigate={navigate}>
                ...
                <div className="text-right">
                  <p
                    ...
                    style={{
                      color:
                        (d.attendance_rate || 0) >= 75
                          ? '#10b981'
                          : (d.attendance_rate || 0) >= 50
                            ? '#f59e0b'
                            : '#ef4444'
                    }}
                  >
                    {R(d.attendance_rate || 0)}%
                  </p>
                </div>
              </Row>
            ))}
          ...
          <div>
            ...
            <div className="p-4 rounded-lg bg-slate-50 dark:bg-slate-700/30 text-center">
              ...
              <p className="text-[10px] text-slate-400 mt-1">
                Marriage anniversaries are not tracked in the current database.
              </p>
            </div>
          </div>
        </div>
          ...
          {audit.slice(0, 12).map((a, i) => (
            ...
            >
              ...
              <span className="text-xs text-slate-700 dark:text-slate-200 flex-1 truncate">
                {a.action || a.description || a.entity || 'System activity'}
              </span>
              <span className="text-[10px] text-slate-400 shrink-0">
                {fmtDate(a.created_at || a.timestamp)}
              </span>
            </button>
          ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {hallOfFame.kpis?.topMember && (
              ...
              >
                ...
                <p className="text-xs font-bold text-slate-900 dark:text-white truncate">
                  {hallOfFame.kpis.topMember.full_name || hallOfFame.kpis.topMember.name}
                </p>
                <p className="text-[10px] text-slate-400">
                  {R(hallOfFame.kpis.topMember.overallScore ?? hallOfFame.kpis.topMember.score)}
                </p>
              </button>
            )}
            {hallOfFame.kpis?.topLeader && (
              ...
              >
                ...
                <p className="text-xs font-bold text-slate-900 dark:text-white truncate">
                  {hallOfFame.kpis.topLeader.leader_name ||
                    hallOfFame.kpis.topLeader.full_name ||
                    hallOfFame.kpis.topLeader.name}
                </p>
                <p className="text-[10px] text-slate-400">
                  {R(hallOfFame.kpis.topLeader.overallScore ?? hallOfFame.kpis.topLeader.score)}
                </p>
              </button>
            )}
            ...
            {hallOfFame.kpis?.bestAttendanceSection && (
              <button
                ...
                onClick={() =>
                  navigate(
                    `/admin/sections?profile=${hallOfFame.kpis.bestAttendanceSection.id || ''}`
                  )
                }
                className="group p-3 rounded-xl bg-slate-50 dark:bg-slate-700/30 text-center hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:ring-1 hover:ring-blue-200 dark:hover:ring-blue-800 transition-all cursor-pointer"
              >
                ...
                <p className="text-xs font-bold text-slate-900 dark:text-white truncate">
                  {hallOfFame.kpis.bestAttendanceSection.name}
                </p>
              </button>
            )}
      ...
      <SectionCard title="AI Executive Summary" icon={Sparkles}>
        <div className="space-y-2">
          ...
          {asArray(summary?.recommendations)
            .slice(0, 3)
            .map((r, i) => (
              <div key={`r${i}`} className="flex items-start gap-2">
                <Zap className="w-3.5 h-3.5 text-violet-500 mt-0.5 shrink-0" />
                <p className="text-xs text-slate-600 dark:text-slate-300">
                  <span className="font-semibold">Recommend:</span> {r.recommendation}
                </p>
              </div>
            ))}
          {aiInsights.length === 0 && asArray(summary?.recommendations).length === 0 && (
            <InsufficientData />
          )}
        </div>
      </SectionCard>
          ...
          <Metric
            ...
            value={
              health?.database?.status === 'connected'
                ? 'Online'
                : health?.database?.status || INSUFFICIENT
            }
            status={health?.database?.status === 'connected' ? 'excellent' : 'critical'}
          />
          <Metric
            label="Last Backup"
            value={backup?.last_backup_at ? fmtDate(backup.last_backup_at) : INSUFFICIENT}
            status={backup?.warning ? 'attention' : 'good'}
          />
          <Metric
            label="Sync Status"
            value={health?.status === 'ok' ? 'Synced' : health?.status || INSUFFICIENT}
            status={health?.status === 'ok' ? 'excellent' : 'watch'}
          />
          <Metric
            label="Pending Subs"
            value={pendingSubs != null ? num(pendingSubs) : INSUFFICIENT}
            status={pendingSubs > 0 ? 'attention' : 'good'}
          />
          <Metric
            label="Notifications"
            value={num(notifCount)}
            status={notifCount > 0 ? 'watch' : 'good'}
          />
          <Metric
            label="Server"
            value={health?.status === 'ok' ? 'Healthy' : health?.status || INSUFFICIENT}
            status={health?.status === 'ok' ? 'excellent' : 'watch'}
          />
        </div>
        <p className="text-[10px] text-slate-400 mt-2">
          Uptime: {health?.uptime || INSUFFICIENT} · Active sections:{' '}
          {num(snap.church?.activeSections)} · Active leaders: {num(snap.church?.activeLeaders)}
        </p>
      </SectionCard>
    </div>
  );
