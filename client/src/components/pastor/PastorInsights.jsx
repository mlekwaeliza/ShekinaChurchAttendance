...
const PastorInsights = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [predRes, anomRes, streakRes, leaderRes, engageRes, demoRes, yoyRes, retRes] =
          await Promise.all([
  ...
  const engagementColumns = [
    {
      accessor: 'member_id',
      header: 'Rank',
      render: (_, idx) => <span className="text-slate-400 font-medium">#{idx + 1}</span>,
      sortable: false,
      align: 'center'
    },
    {
    ...
    { accessor: 'section_name', header: 'Section', sortable: true },
    {
      ...
      header: 'Consistency (90d)',
      sortable: true,
      align: 'center',
      render: (row) => `${row.consistency}%`
    },
    {
      accessor: 'streak',
  ...
  const leaderColumns = [
    ...
    { accessor: 'section_name', header: 'Section', sortable: true },
    {
      ...
      sortable: true,
      align: 'center',
      render: (row) => <span className="font-medium">{row.avg_rate}%</span>
    },
    {
      accessor: 'trend_direction',
      header: 'Trend',
      sortable: true,
      align: 'center',
      render: (row) => {
        if (row.trend_direction === 'improving')
          return (
            <Badge variant="success" className="gap-1 flex justify-center">
              <TrendingUp className="w-3 h-3" /> Improving
            </Badge>
          );
        if (row.trend_direction === 'declining')
          return (
            <Badge variant="danger" className="gap-1 flex justify-center">
              Declining
            </Badge>
          );
        return (
          <Badge variant="default" className="gap-1 flex justify-center">
            Stable
          </Badge>
        );
      }
          ...
          {payload.map((entry, index) => (
            <div key={index} className="flex items-center gap-2 mb-1">
              ...
              <span className="text-sm font-medium text-slate-600 dark:text-slate-300">
                {entry.name}:{' '}
                <span className="text-slate-900 dark:text-slate-100 font-bold">{entry.value}</span>
    ...
    <div className="space-y-8 animate-fade-in">
          ...
          <div className="relative z-10 flex items-start gap-4">
            ...
            <div>
              <h3 className="text-lg font-bold mb-1">
                Attention Required: Attendance Drops Detected
              </h3>
              <p className="text-white/90 text-sm mb-4">
                The following sections have dropped significantly below their 90-day average
                attendance rate.
              </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <ChartCard
          ...
          empty={
            !demographics ||
            (demographics.gender.length === 0 && demographics.age_group.length === 0)
          }
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 h-full gap-4">
            <div className="h-full flex flex-col justify-center relative">
              <h4 className="text-xs font-semibold text-slate-500 absolute top-0 w-full text-center">
                Gender
              </h4>
          ...
          <div className="space-y-3">
            {streaks && streaks.length > 0 ? (
              streaks.map((streak, idx) => (
                <div
                  key={idx}
                  className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 flex items-center justify-between hover:border-amber-300 dark:hover:border-amber-700 transition-colors shadow-sm"
                >
        ...
        <div className="xl:col-span-2 space-y-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">
                Member Engagement Score Tracker
              </h3>
            </div>
            <DataTable
